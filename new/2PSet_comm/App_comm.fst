...
let resolve_conflict (x:op_t) (y:op_t{fst x <> fst y}) : resolve_conflict_res = 
  match x, y with
  |(_, Rem ele), (_, Add ele1) -> Second_then_first
  |(_, Add ele), (_, Rem ele1) -> First_then_second
  |_ -> Second_then_first
...
let rec lem_l2r_s10 (lca s1 s2:st) (last2:op_t)
 : Lemma (requires is_prefix (ops_of lca) (ops_of s2) /\
                   ops_of s1 = ops_of lca /\
                   Rem? (snd last2))
...
let rec lem_l2r'_rr_eq (lca s1 s2:st) (last1 last2:op_t)
  : Lemma (requires consistent_branches lca s1 s2 /\
  ...
  if ops_of s1 = ops_of lca then ()
    ...
    (let s1' = inverse_st s1 in
     pre1_pre2_s1 lca s1 s2;   
     lem_l2r'_rr_eq lca s1' s2 last1 last2)
