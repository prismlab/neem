...
let add_lastop (lca s1:st) (last1:op_t)
  : Lemma (requires is_prefix (ops_of lca) (ops_of s1) /\
                    ...
                    (exists l. v_of s1 == apply_log (v_of lca) l)) 
          (ensures (exists l. do (v_of s1) last1 == apply_log (v_of lca) l)) = 
  ...
  un_snoc_snoc (ops_of s1) last1;
  assert (fst (un_snoc (snoc (ops_of s1) last1)) = (ops_of s1));
  assert (apply_log init_st (snoc (ops_of s1) last1) == do (apply_log init_st (ops_of s1)) last1); 
  assert (do (v_of s1) last1 == apply_log init_st (snoc (ops_of s1) last1));
  assert (do (v_of s1) last1 == apply_log (v_of lca) (diff (snoc (ops_of s1) last1) (ops_of lca))); ()
...
let rec linearizable_s1_0'' (lca s1 s2':st) (last2:op_t)
  : Lemma (requires consistent_branches lca s1 s2' /\
          ...
          (ensures (exists l2. (do (v_of s2') last2 == apply_log (v_of lca) l2)) /\
                   eq (do (v_of s2') last2) (concrete_merge (v_of lca) (v_of s1) (do (v_of s2') last2)))
  ...
  if ops_of s2' = ops_of lca then
    ...
    split_prefix init_st (ops_of lca) (ops_of s2');
    add_lastop lca s2' last2; // ((exists l2. (do (v_of inv2) last2 == apply_log (v_of lca) l2))); 
    linearizable_s1_0'' lca s1 inv2 last2;
...
let linearizable_s1_0' (lca s1 s2:st)
  : Lemma (requires consistent_branches_s2_gt0 lca s1 s2 /\
                    ops_of s1 = ops_of lca)
          (ensures (exists l1. v_of s1 == apply_log (v_of lca) l1) /\
...
let rec linearizable_s2_0'' (lca s1' s2:st) (last1:op_t)
  : Lemma (requires consistent_branches lca s1' s2 /\
                    ...
                    fst last1 > 0 /\
                    (forall id. mem_id id (ops_of lca) ==> lt id (fst last1)))
          (ensures (exists l1. (do (v_of s1') last1 == apply_log (v_of lca) l1)) /\
                   eq (do (v_of s1') last1) (concrete_merge (v_of lca) (do (v_of s1') last1) (v_of s2)))
  ...
  if ops_of s1' = ops_of lca then
    ...
    split_prefix init_st (ops_of lca) (ops_of s1');
    add_lastop lca s1' last1; // ((exists l1. (do (v_of s1') last1 == apply_log (v_of lca) l1)));
...
let linearizable_s2_0' (lca s1 s2:st)
  ...
  split_prefix init_st (ops_of lca) (snoc pre1 last1); 
  lem_inverse (ops_of lca) (ops_of s1);
  lastop_diff (ops_of lca) (ops_of s1);
  inverse_diff_id_s1' (ops_of lca) (ops_of s1) (ops_of s2);
...
let rec linearizable_gt0_s2'_s10 (lca s1 s2:st) (last1 last2:op_t)
  : Lemma (requires consistent_branches lca s1 s2 /\
          ...
          (decreases  %[length (ops_of s2)]) = 
  split_prefix init_st (ops_of lca) (ops_of s2); // (exists l2. (v_of s2) == apply_log (v_of lca) l2);
  add_lastop lca s2 last2; //(exists l2. (do (v_of s2) last2 == apply_log (v_of lca) l2)); 
  add_lastop lca s1 last1; //(exists l1. (do (v_of s1) last1 == apply_log (v_of lca) l1)));
...
let rec linearizable_gt0_s2' (lca s1 s2:st) (last1 last2:op_t)
  ...
  split_prefix init_st (ops_of lca) (ops_of s2); // (exists l2. (v_of s2) == apply_log (v_of lca) l2);
  split_prefix init_st (ops_of lca) (ops_of s1); // (exists l1. (v_of s1) == apply_log (v_of lca) l1);
  add_lastop lca s2 last2; //(exists l2. (do (v_of s2) last2 == apply_log (v_of lca) l2)); 
  add_lastop lca s1 last1; //(exists l1. (do (v_of s1) last1 == apply_log (v_of lca) l1)));
...
let rec linearizable_gt0_s1' (lca s1 s2:st) (last1 last2:op_t)
  : Lemma (requires consistent_branches lca s1 s2 /\
                    fst last1 <> fst last2 /\
                    //return (v_of s2) last2 == ret_of last2 /\
                    //return (v_of s1) last1 == ret_of last1 /\
                    distinct_ops (snoc (ops_of s1) last1) /\
                    distinct_ops (snoc (ops_of s2) last2) /\
                    First_then_second? (resolve_conflict last1 last2))
          (ensures (exists l2. (do (v_of s2) last2 == apply_log (v_of lca) l2)) /\
...
let linearizable_gt0 (lca s1 s2:st)
  : Lemma (requires consistent_branches_s1s2_gt0 lca s1 s2 /\
                    ...
                    (exists l2. (v_of s2) == apply_log (v_of lca) l2) /\
                    (exists l1. (v_of s1) == apply_log (v_of lca) l1) /\
                    ((First_then_second? (resolve_conflict last1 last2) \/ Noop_first? (resolve_conflict last1 last2)) ==>
                       (exists l1. (v_of (inverse_st s1) == apply_log (v_of lca) l1))) /\

                    ((Second_then_first? (resolve_conflict last1 last2) \/ Noop_second? (resolve_conflict last1 last2)) ==>
                       (exists l2. (v_of (inverse_st s2) == apply_log (v_of lca) l2))) /\

                    ((First? (resolve_conflict last1 last2) \/ Noop_both? (resolve_conflict last1 last2)) ==>
                       (exists l1. (v_of (inverse_st s1) == apply_log (v_of lca) l1)) /\
                       (exists l2. (v_of (inverse_st s2) == apply_log (v_of lca) l2)))))
          (ensures (let _, last1 = un_snoc (ops_of s1) in
                    let _, last2 = un_snoc (ops_of s2) in
                    ...
                    (Noop_first? (resolve_conflict last1 last2) ==>
                     eq (concrete_merge (v_of lca) (v_of (inverse_st s1)) (v_of s2))
                        (concrete_merge (v_of lca) (v_of s1) (v_of s2))) /\

                    (Noop_second? (resolve_conflict last1 last2) ==>
                     eq (concrete_merge (v_of lca) (v_of s1) (v_of (inverse_st s2)))
                        (concrete_merge (v_of lca) (v_of s1) (v_of s2))) /\
                        
                    (Noop_both? (resolve_conflict last1 last2) ==>
                     eq (concrete_merge (v_of lca) (v_of (inverse_st s1)) (v_of (inverse_st s2)))
                        (concrete_merge (v_of lca) (v_of s1) (v_of s2))))) = 
  let _, last1 = un_snoc (ops_of s1) in
  let _, last2 = un_snoc (ops_of s2) in
  ...
  else if Noop_first? (resolve_conflict last1 last2) then
    linearizable_gt0_s1'_noop lca s1 s2 
  else if Noop_second? (resolve_conflict last1 last2) then
    linearizable_gt0_s2'_noop lca s1 s2 
  else if Noop_both? (resolve_conflict last1 last2) then
    linearizable_gt0_s1's2'_noop_both lca s1 s2
  else linearizable_gt0_s1's2' lca s1 s2
...
let interleaving_helper_inv1inv2 (lca s1 s2 l':log)
  : Lemma
      ...
      (ensures (let _, last1 = un_snoc s1 in
                is_interleaving (Seq.snoc l' last1) (diff s1 lca) (diff s2 lca)))
  = assert (is_interleaving l' (fst (Seq.un_snoc (diff s1 lca))) (fst (Seq.un_snoc (diff s2 lca))));
    let _, last1 = Seq.un_snoc (diff s1 lca) in
    let l = Seq.snoc l' last1 in
    assert (is_interleaving l (diff s1 lca) (diff s2 lca)); ()

let interleaving_helper_inv1inv2_noop_both (lca s1 s2 l':log)
  : Lemma
      (requires Seq.length s2 > 0 /\ Seq.length s1 > 0 /\
                is_prefix lca (fst (Seq.un_snoc s2)) /\ is_prefix lca s1 /\
                is_prefix lca (fst (Seq.un_snoc s1)) /\ is_prefix lca s2 /\
                is_interleaving l' (diff (fst (Seq.un_snoc s1)) lca) (diff (fst (Seq.un_snoc s2)) lca))
      (ensures (is_interleaving l' (diff s1 lca) (diff s2 lca)))
  = assert (is_interleaving l' (fst (Seq.un_snoc (diff s1 lca))) (fst (Seq.un_snoc (diff s2 lca))));
    let l = l' in
    assert (is_interleaving l (diff s1 lca) (diff s2 lca)); ()

let interleaving_helper_inv1_noop (lca s1 s2 l':log)
  : Lemma
      (requires Seq.length s1 > 0 /\
                is_prefix lca (fst (Seq.un_snoc s1)) /\ is_prefix lca s2 /\
                is_interleaving l' (diff (fst (Seq.un_snoc s1)) lca) (diff s2 lca))
  ...
  = assert (is_interleaving l' (fst (Seq.un_snoc (diff s1 lca))) (diff s2 lca));
    let l = l' in
    assume (is_interleaving l (diff s1 lca) (diff s2 lca)); ()

let interleaving_helper_inv2_noop (lca s1 s2 l':log)
  : Lemma
      (requires Seq.length s2 > 0 /\
                is_prefix lca (fst (Seq.un_snoc s2)) /\ is_prefix lca s1 /\
                is_interleaving l' (diff s1 lca) (diff (fst (Seq.un_snoc s2)) lca)) 
...
let interleaving_s1s2_inv (lca s1 s2:st) (l':log)
  : Lemma (requires consistent_branches_s1s2_gt0 lca s1 s2 /\
                    is_prefix (ops_of lca) (ops_of (inverse_st s1)) /\
                    is_prefix (ops_of lca) (ops_of (inverse_st s2)) /\
          ...
          (ensures (let _, last2 = un_snoc (ops_of s2) in
                    ...
                    interleaving_predicate l lca s1 s2 /\
                    (exists l. interleaving_predicate l lca s1 s2))) = 
  let pre2, _ = un_snoc (ops_of s2) in
  let pre1, last1 = un_snoc (ops_of s1) in
  let l = Seq.snoc l' last1 in
  interleaving_helper_inv1inv2 (ops_of lca) (ops_of s1) (ops_of s2) l'; 
  linearizable_gt0 lca s1 s2;
  symmetric (apply_log (v_of lca) l')
            (concrete_merge (v_of lca) (v_of (inverse_st s1)) (v_of (inverse_st s2)));
  lem_do (concrete_merge (v_of lca) (v_of (inverse_st s1)) (v_of (inverse_st s2))) (apply_log (v_of lca) l') last1;
  symmetric (do (concrete_merge (v_of lca) (v_of (inverse_st s1)) (v_of (inverse_st s2))) last1) (do (apply_log (v_of lca) l') last1);
  inverse_helper (v_of lca) l' last1;
  eq_is_equiv (apply_log (v_of lca) l) (do (apply_log (v_of lca) l') last1);
  transitive (apply_log (v_of lca) l) (do (concrete_merge (v_of lca) (v_of (inverse_st s1)) (v_of (inverse_st s2))) last1)
             (concrete_merge (v_of lca) (v_of s1) (v_of s2))

let interleaving_s1s2_inv_noop (lca s1 s2:st) (l':log)
  : Lemma (requires consistent_branches_s1s2_gt0 lca s1 s2 /\
                    is_prefix (ops_of lca) (ops_of (inverse_st s1)) /\
                    is_prefix (ops_of lca) (ops_of (inverse_st s2)) /\
                    interleaving_predicate l' lca (inverse_st s1) (inverse_st s2) /\
          ...
          (ensures (let l = l' in
                    ...
                    (exists l. interleaving_predicate l lca s1 s2))) = 
  let l = l' in
  interleaving_helper_inv1inv2_noop_both (ops_of lca) (ops_of s1) (ops_of s2) l'; 
  linearizable_gt0 lca s1 s2;
  symmetric (apply_log (v_of lca) l')
            (concrete_merge (v_of lca) (v_of (inverse_st s1)) (v_of (inverse_st s2)));
  symmetric (concrete_merge (v_of lca) (v_of (inverse_st s1)) (v_of (inverse_st s2))) (apply_log (v_of lca) l');
  eq_is_equiv (apply_log (v_of lca) l) (apply_log (v_of lca) l');
  transitive (apply_log (v_of lca) l) (concrete_merge (v_of lca) (v_of (inverse_st s1)) (v_of (inverse_st s2)))
             (concrete_merge (v_of lca) (v_of s1) (v_of s2))

let interleaving_s1_inv_noop (lca s1 s2:st) (l':log)
  : Lemma (requires consistent_branches_s1s2_gt0 lca s1 s2 /\
                    is_prefix (ops_of lca) (ops_of (inverse_st s1)) /\
                    interleaving_predicate l' lca (inverse_st s1) s2 /\
                    (let _, last1 = un_snoc (ops_of s1) in
          ...
          (ensures (let l = l' in
                    ...
                    (exists l. interleaving_predicate l lca s1 s2))) = 
  let l = l' in
  interleaving_helper_inv1_noop (ops_of lca) (ops_of s1) (ops_of s2) l'; 
  linearizable_gt0 lca s1 s2;
  symmetric (apply_log (v_of lca) l')
            (concrete_merge (v_of lca) (v_of (inverse_st s1)) (v_of s2));
  symmetric (concrete_merge (v_of lca) (v_of (inverse_st s1)) (v_of s2)) (apply_log (v_of lca) l');
  eq_is_equiv (apply_log (v_of lca) l) (apply_log (v_of lca) l');
  transitive (apply_log (v_of lca) l) (concrete_merge (v_of lca) (v_of (inverse_st s1)) (v_of s2))
             (concrete_merge (v_of lca) (v_of s1) (v_of s2))

let interleaving_s2_inv_noop (lca s1 s2:st) (l':log)
  : Lemma (requires consistent_branches_s1s2_gt0 lca s1 s2 /\
                    is_prefix (ops_of lca) (ops_of (inverse_st s2)) /\
                    interleaving_predicate l' lca s1 (inverse_st s2) /\
                    (let _, last1 = un_snoc (ops_of s1) in
          ...
          (ensures (let l = l' in
                    ...
                    (exists l. interleaving_predicate l lca s1 s2))) = 
  let l = l' in
  interleaving_helper_inv2_noop (ops_of lca) (ops_of s1) (ops_of s2) l'; 
  linearizable_gt0 lca s1 s2;
  symmetric (apply_log (v_of lca) l')
            (concrete_merge (v_of lca) (v_of s1) (v_of (inverse_st s2)));
  symmetric (concrete_merge (v_of lca) (v_of s1) (v_of (inverse_st s2))) (apply_log (v_of lca) l');
  eq_is_equiv (apply_log (v_of lca) l) (apply_log (v_of lca) l');
  transitive (apply_log (v_of lca) l) (concrete_merge (v_of lca) (v_of s1) (v_of (inverse_st s2)))
             (concrete_merge (v_of lca) (v_of s1) (v_of s2))
...
let linearizable_s1s2_gt0_pre (lca s1 s2:st)
  : Lemma (requires consistent_branches_s1s2_gt0 lca s1 s2 /\
                    (let _, last1 = un_snoc (ops_of s1) in
                     let _, last2 = un_snoc (ops_of s2) in
                     fst last1 <> fst last2))
          (ensures (let inv2 = inverse_st s2 in 
                    ...
                    (forall id id1. mem_id id (ops_of lca) /\ mem_id id1 (diff (ops_of inv1) (ops_of lca)) ==> lt id id1) /\
                    (forall id id1. mem_id id (ops_of lca) /\ mem_id id1 (diff (ops_of inv2) (ops_of lca)) ==> lt id id1) /\
                    (forall id. mem_id id (diff (ops_of inv1) (ops_of lca)) ==> not (mem_id id (diff (ops_of inv2) (ops_of lca)))))) 
  = let inv2 = inverse_st s2 in 
    let inv1 = inverse_st s1 in 
    lem_inverse (ops_of lca) (ops_of s1);
    lem_inverse (ops_of lca) (ops_of s2);
      ...
      linearizable_s2_0 lca s1 s2
        ...
        begin
          ...
          with _. begin
            ...
            with l
            and begin
              interleaving_s1_inv_noop lca s1 s2 l'
            end
          end
        end
        ...
        begin
          ...
          with _. begin
            ...
            with l
            and begin
              interleaving_s2_inv_noop lca s1 s2 l'
            end
          end
        end
        ...
        begin
          linearizable_s1s2_gt0_pre lca s1 s2;
          split_prefix init_st (ops_of lca) pre1;
          split_prefix init_st (ops_of lca) pre2;
          ...
          with _. begin
            ...
            with l
            and begin
              interleaving_s1s2_inv_noop lca s1 s2 l'
            end
          end
        end
        ...
        begin
          assert (First? (resolve_conflict last1 last2));
          linearizable_s1s2_gt0_pre lca s1 s2;
          split_prefix init_st (ops_of lca) pre1;
          split_prefix init_st (ops_of lca) pre2;
          linearizable lca inv1 inv2;
          ...
          with _. begin
            ...
            with l
            and begin
              interleaving_s1s2_inv lca s1 s2 l'
            end
          end
        end
      end
