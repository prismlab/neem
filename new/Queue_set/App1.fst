...
let two_op_same_id' (l:log) (op op1:op_t)
  : Lemma (requires mem op l /\ mem op1 l /\ fst op = fst op1 /\ snd op <> snd op1)
          ...
          (decreases length l) =
        //  [SMTPat (distinct_ops l)] =  //uncomment later
 match length l with
  |0 -> ()
  |1 -> ()
  |_ -> if head l = op then 
...
let two_op_same_id (l:log)
  ...
  admit() //smtpat based on prev proof, proved, for now admitted
...
let rec linearizable_gt0_ind1_dd_stf (lca s1' s2':st) (last1 last2:op_t)
  ...
  if ops_of s1' = ops_of lca then admit()
    ...
    (let s1'' = inverse_st s1' in
     ...
     last_deq (v_of s1'') last1'; 
     valid_is_unique s1'';
     assert (apply_log init_st pre1 == v_of s1'');
     assert (apply_log init_st (ops_of s1') == v_of s1');
     assert (v_of s1'' == apply_log init_st (fst (un_snoc (ops_of s1'))));
     lem_last (ops_of s1');
     assume (last (ops_of s1') = last1'); admit();
     assert ((v_of s1') == do (v_of s1'') last1'); admit();
     if Dequeue? (fst (snd last1')) then
       (if not (S.mem min2 (v_of lca)) && S.mem min1 (v_of lca) then admit()
          ...
          (admit();assume (consistent_branches lca s1' (do_st s2' last2) /\ //todo
                   consistent_branches lca s1'' s2' /\    //todo
                   consistent_branches lca s1' s2'); //todo
           linearizable_gt0_ind1_dd_stf lca s1'' s2' last1' last2))
...
let rec linearizable_gt0_ind1_dd_stf (lca s1' s2':st) (last1 last2:op_t)
  ...
  if ops_of s1' = ops_of lca then admit()
    ...
    (let s1'' = inverse_st s1' in
     ...
     last_deq (v_of s1'') last1'; 
     if Dequeue? (fst (snd last1')) then
       (admit();if not (S.mem min2 (v_of lca)) && S.mem min1 (v_of lca) then admit()
        else 
          (assume (consistent_branches lca s1' (do_st s2' last2) /\ //todo
                   consistent_branches lca s1'' s2' /\    //todo
                   consistent_branches lca s1' s2'); //todo
       ...
       (if S.mem min2 (v_of lca) && S.mem min1 (v_of lca) then
          (admit();if (fst last1', get_ele last1') <> min1 then admit() //this case not possible because of commutativity of last1' and last1
           else 
             (assume ((fst last1', get_ele last1') = min1); 
              lem_foldl init_st (ops_of lca);
              assume (S.mem_id_s (fst min1) (v_of lca)); 
              assume (mem_id (fst min1) (ops_of lca));
              concrete_id_lt lca s1'))
            
       else if not (S.mem min1 (v_of lca)) && not (S.mem min2 (v_of lca)) then 
         (admit();assume (S.mem min2 (v_of s2'));
          assume (S.mem min2 (concrete_merge (v_of lca) (do (v_of s1') last1) (v_of s2')));
          if (fst last1', get_ele last1') = min1 then 
            (assume ((v_of s1'') = S.empty); //assumption - because there won't be an Enq(z) in s1'' - lca s.t. it is not followed by Deq->z. If there exists such an Enq(z), it will be brought down as Enq(z) and Deq->x are commutative.
             assert (forall e. S.mem e (v_of s1') <==> e = (fst last1', get_ele last1')); 
             assert (min1 = (fst last1', get_ele last1')); 
             assert ((do (v_of s1') last1) = S.empty);
             ()) //done
          else admit()) //this case not possible because of commutativity of last1' and last1
        
      
       else if S.mem min1 (v_of lca) && not (S.mem min2 (v_of lca)) then
         (admit();assume (S.mem min2 (v_of s2')); 
          assume (S.mem min2 (concrete_merge (v_of lca) (do (v_of s1') last1) (v_of s2'))); 
          if (fst last1', get_ele last1') = min1 then 
           (lem_foldl init_st (ops_of lca);
         ...
         (admit();assume (not (S.mem min1 (v_of lca)) /\ S.mem min2 (v_of lca));
          concrete_id_lt lca s1';
          concrete_id_lt lca s2';
          let da = S.remove_if (v_of s1') (fun e -> S.mem e (v_of lca)) in
          assert (S.mem min1 da);
