...
let resolve_conflict (lca:concrete_st) (x:op_t) (y:op_t{fst x <> fst y}) : resolve_conflict_res =
  ...
  |(_,(Dequeue,Some _)), (_,(Dequeue,Some _)) -> 
                    if ret_ele x = ret_ele y then First
                    else (if ((S.mem (ret_ele x) lca && S.mem (ret_ele y) lca) ||
                             (not (S.mem (ret_ele x) lca) && not (S.mem (ret_ele y) lca))) then
                             (if fst (ret_ele x) >= fst (ret_ele y) then First_then_second
                              else Second_then_first)
                         else if S.mem (ret_ele x) lca then Second_then_first
                         else (assert (not (S.mem (ret_ele x) lca) && S.mem (ret_ele y) lca); First_then_second))
...
let dd_stf_x_in_lca_y_notin_lca (lca s1' s2':st) (last1 last2:op_t)
  : Lemma (requires apply_log_ret init_st (snoc (ops_of s1') last1) /\
                    apply_log_ret init_st (snoc (ops_of s2') last2) /\
                    do_pre (v_of s1') last1 /\ do_pre (v_of s2') last2 /\ 
                    //consistent_branches_s1s2_gt0 lca (do_st s1' last1) (do_st s2' last2) /\
                    ...
                    Dequeue? (fst (snd last2)) /\ Some? (ret_of last2) /\
                    fst (ret_ele last1) < fst (ret_ele last2) /\
                    S.mem (ret_ele last1) (v_of lca) /\
                    ~ (S.mem (ret_ele last2) (v_of lca) /\
  ...
  if ops_of s1' = ops_of lca then admit()
    ...
    (let s1'' = inverse_st s1' in
     ...
     if Enqueue? (fst (snd last1')) then 
       (if (fst last1', get_ele last1') <> min1 then admit()
        else 
  ...
  if ops_of s1' = ops_of lca then admit()
    ...
    (let s1'' = inverse_st s1' in
     ...
     if Dequeue? (fst (snd last1')) then
       (admit();if S.mem min1 (v_of lca) && not (S.mem min2 (v_of lca)) then 
         (let da = S.remove_if (v_of s1'') (fun e -> S.mem e (v_of lca)) in
       ...
       (assert (Enqueue? (fst (snd last1'))); 
        if (fst last1', get_ele last1') <> min1 then admit() //this case not possible because of commutativity of last1' and last1
          ...
          (if S.mem min1 (v_of lca) then
             ...
             (admit();if S.mem min2 (v_of lca) then // not (S.mem min1 (v_of lca))
