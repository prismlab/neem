...
//conflict resolution
let resolve_conflict (lca:concrete_st) (x:op_t) (y:op_t{fst x <> fst y}) : resolve_conflict_res =
  match x, y with
...
let resolve_conflict_prop (lca:concrete_st) (x:op_t) (y:op_t{fst x <> fst y})
  : Lemma (ensures ((resolve_conflict lca x y = First_then_second) <==>
                    ((Dequeue? (fst (snd x)) /\ Dequeue? (fst (snd y)) /\
                      Some? (ret_of x) /\ Some? (ret_of y) /\ ret_ele x <> ret_ele y /\ fst (ret_ele x) > fst (ret_ele y))) \/
                     (Enqueue? (fst (snd x)) /\ Enqueue? (fst (snd y)) /\ fst x > fst y) \/
                     (Enqueue? (fst (snd x)) /\ Dequeue? (fst (snd y)) /\ Some? (ret_of y))) /\
                     
                   (resolve_conflict lca x y = Noop_second ==> (Dequeue? (fst (snd y)) /\ None? (ret_of y))) /\
                  
                   (resolve_conflict lca x y = Noop_first ==> (Dequeue? (fst (snd x)) /\ None? (ret_of x))) /\
                   
                   (resolve_conflict lca x y = Noop_both <==> 
                     Dequeue? (fst (snd y)) /\ None? (ret_of y) /\ Dequeue? (fst (snd x)) /\ None? (ret_of x)) /\
                     
                   (resolve_conflict lca x y = First <==>
                     Dequeue? (fst (snd x)) /\ Dequeue? (fst (snd y)) /\
                      Some? (ret_of x) /\ Some? (ret_of y) /\ ret_ele x = ret_ele y) /\
                      
                   (resolve_conflict lca x y = Second_then_first <==>
                    ((Dequeue? (fst (snd x)) /\ Dequeue? (fst (snd y)) /\ 
                      Some? (ret_of x) /\ Some? (ret_of y) /\ ret_ele x <> ret_ele y /\ fst (ret_ele x) <= fst (ret_ele y))) \/
                     (Enqueue? (fst (snd x)) /\ Enqueue? (fst (snd y)) /\ fst x < fst y) \/
                     (Enqueue? (fst (snd y)) /\ Dequeue? (fst (snd x)) /\ Some? (ret_of x)))) = ()
...
let linearizable_gt0_ind_fts (lca s1 s2:st) (last1 last2:op_t)
  : Lemma (requires ind_fts_pre lca s1 s2 last1 last2)
          ...
          [SMTPat (eq (do (concrete_merge (v_of lca) (v_of s1) (do (v_of s2) last2)) last1)
                      (concrete_merge (v_of lca) (do (v_of s1) last1) (do (v_of s2) last2)))] =
  resolve_conflict_prop (v_of lca) last1 last2;
...
let rec dd_lt (lca s1 s2:st) (last1 last2:op_t)
  : Lemma (requires consistent_branches_s1s2_gt0 lca (do_st s1 last1) (do_st s2 last2) /\
                    consistent_branches lca s1 s2 /\
                    consistent_branches lca (do_st s1 last1) s2 /\
                    ret_of last1 = return (v_of s1) last1 /\
                    ...
                    Dequeue? (fst (snd last2)) /\ Some? (ret_of last2) /\
                    fst (ret_ele last1) < fst (ret_ele last2) /\
                    (S.mem (ret_ele last1) (v_of lca) /\ S.mem (ret_ele last2) (v_of lca)))
          (ensures eq (do (concrete_merge (v_of lca) (do (v_of s1) last1) (v_of s2)) last2)
                      (concrete_merge (v_of lca) (do (v_of s1) last1) (do (v_of s2) last2)))
...
let linearizable_gt0_s1'_noop (lca s1 s2:st) (last1 last2:op_t)
  : Lemma (requires consistent_branches_s1s2_gt0 lca (do_st s1 last1) (do_st s2 last2) /\ 
                    consistent_branches lca s1 (do_st s2 last2) /\
                    fst last1 <> fst last2 /\
                    ret_of last1 == return (v_of s1) last1 /\
...
let linearizable_gt0_s2'_noop (lca s1 s2:st) (last1 last2:op_t)
  : Lemma (requires consistent_branches_s1s2_gt0 lca (do_st s1 last1) (do_st s2 last2) /\ 
                    consistent_branches lca (do_st s1 last1) s2 /\
                    fst last1 <> fst last2 /\
                    ret_of last1 == return (v_of s1) last1 /\
...
let linearizable_gt0_s1's2'_noop_both (lca s1 s2:st) (last1 last2:op_t)
  : Lemma (requires consistent_branches_s1s2_gt0 lca (do_st s1 last1) (do_st s2 last2) /\ 
                    consistent_branches lca s1 s2 /\
                    fst last1 <> fst last2 /\
                    ret_of last1 == return (v_of s1) last1 /\
