...
let min_union (s s1:concrete_st)
  : Lemma (requires (s <> S.empty /\ Some? (S.find_min s) /\ S.unique_st s /\ 
                    (forall e. S.mem e s1 ==> lt (fst (S.extract_s (S.find_min s))) (fst e))))
...
let s1s2_gt_lca (lca s1 s2:st1)
  : Lemma (requires consistent_branches lca s1 s2 /\
          ...
          (ensures (forall e e1. S.mem e (v_of lca) /\ 
                            S.mem e1 (S.remove_if (v_of s1) (fun e -> S.mem e (v_of lca))) ==> lt (fst e) (fst e1)) /\
                   (forall e e1. S.mem e (v_of lca) /\ 
                            S.mem e1 (S.remove_if (v_of s2) (fun e -> S.mem e (v_of lca))) ==> lt (fst e) (fst e1))) =
    ...
    (if S.mem min2 (v_of lca) && S.mem min1 (v_of lca) then
       (assume (length (ops_of s1) > length (ops_of lca)); 
        ...
        if Dequeue? (fst (snd last1')) && ret_of last1 = return (v_of s1') last1 then
          (admit();assume (consistent_branches lca (do_st s1' last1) (do_st s2 last2) /\ //todo
           ...
           (lem_apply_log init_st (ops_of s1); S.always_min_exists (v_of s1'); valid_is_unique s1';
            ...
            assume (S.mem min1 (v_of s1')); 
            assume (S.mem min2 (v_of s1')); //todo
            assume (S.mem min2 (do (v_of s1) last1)); 
            assume (S.mem min2 i'); 
            assume (S.find_min i' = Some min2);
            ...
            assume (S.find_min (concrete_merge (v_of lca) (do (v_of s1) last1) (v_of s2)) = Some min2);
            ())
       else admit())
    else admit())
