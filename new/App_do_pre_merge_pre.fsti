...
val get_ts (s:concrete_st) : S.t pos
...
let do_pre (s:concrete_st) (o:op_t) : prop =
  (forall id. S.mem id (get_ts s) ==> fst o > id)
...
val subset_ts (s:concrete_st) (o:op_t) 
  : Lemma (requires do_pre s o)
          (ensures (S.subset (get_ts (do s o)) (S.insert (fst o) (get_ts s))))
...
let subset_pre (s:concrete_st) (l:log{do_pre_prop s l}) : prop =
  (forall id. S.mem id (get_ts (apply_log s l)) ==> S.mem id (get_ts s) \/ mem_id id l)

(*let rec do_subset_prop (x:concrete_st) (l:log{do_pre_prop x l}) : Tot prop (decreases length l) =
  match length l with
  |0 -> True
  |_ -> subset_pre x l /\ 
       (let pre, _ = un_snoc l in do_subset_prop x pre)

let rec lem_subset_prop (x:concrete_st) (l:log)
  : Lemma (requires do_subset_prop x l)
          (ensures (length l > 0 ==> 
                       (let pre, _ = un_snoc l in
                         do_subset_prop x pre))) 
          (decreases length l) =
  match length l with
  |0 -> ()
  |_ -> (let pre, _ = un_snoc l in
        lem_subset_prop x pre;
        assert (do_subset_prop x pre); ())*)

let rec sorted (l:log) : Tot prop (decreases length l) =
  match length l with
  |0 | 1 -> True
  |_ -> let pre, lastop = un_snoc l in
       (forall id. mem_id id pre ==> fst lastop > id) /\ sorted pre

let rec lem_sorted_inv (l:log)
  ...
  match length l with
  |0 | 1 -> ()
  |_ -> let pre, lastop = un_snoc l in lem_sorted_inv pre
...
let valid_st (s:st0) : prop =
  ...
  //do_subset_prop init_st (ops_of s) /\
...
val linearizable_s1_0''_base_merge_pre (lca s1 s2':st) (last2:op_t)
  : Lemma (requires do_pre (v_of s2') last2 /\
                    ...
                    merge_pre (v_of lca) (v_of s1) (do (v_of s2') last2) /\
                    length (ops_of s2') > 0 /\
                    do_pre (v_of (inverse_st lca)) last2)
