...
(*let commu_seq_prop (op:op_t) (l:log)
  : Lemma (requires Add? (snd op))
          (ensures commutative_seq op l <==> ((forall e. mem e l ==> ~ (get_ele op = get_ele e /\ Rem? (snd e))) \/
                                           ~ (exists e. mem e l /\ get_ele e = get_ele op /\ Add? (snd e) /\ fst e >= fst op /\
                                                (let _, suf = pre_suf l e in
                                                 commutative_seq e suf)))) = ()*)

let rec commu_seq_prop (op:op_t) (l:log)
  : Lemma (requires Add? (snd op))
          (ensures commutative_seq op l <==>
                   (~ (exists e. mem e l /\ get_ele e = get_ele op /\ Rem? (snd e)) /\
                   ~ (exists e. mem e l /\ get_ele e = get_ele op /\ Add? (snd e) /\ fst e <> fst op)) )
                 //  ((forall e. mem e l /\ get_ele e = get_ele op ==>
                   //   (Add? (snd e) /\ fst e = fst op))))
          (decreases length l) =
          //[SMTPat (commutative_seq op l)] =
  match length l with
  |0 -> ()
  |1 -> ()
...
let lem_l2a_last'_base_ind (lca s1 s2:st) (last2:op_t)
  : Lemma (requires length (ops_of lca) > 0 /\
                    ops_of s1 = ops_of lca /\ ops_of s2 = ops_of lca /\
                    not (mem_id (fst last2) (ops_of lca)) /\
                    Add? (snd last2) /\
                    ...
                    (let l' = inverse_st lca in
                     let s1' = inverse_st s1 in
                     let s2' = inverse_st s2 in
                     eq (do (concrete_merge (v_of l') (v_of s1') (v_of s2')) last2)
                        (concrete_merge (v_of l') (v_of s1') (do (v_of s2') last2))))
          (ensures eq (do (concrete_merge (v_of lca) (v_of s1) (v_of s2)) last2)
                      (concrete_merge (v_of lca) (v_of s1) (do (v_of s2) last2))) = 
  let pre, lastl = un_snoc (ops_of lca) in
  lem_mem_ele_mem_id_single lastl (ops_of lca);
...
let rec lem_l2a_last'_base (lca s1 s2:st) (last2:op_t)
  : Lemma (requires ops_of s1 = ops_of lca /\ ops_of s2 = ops_of lca /\
                    not (mem_id (fst last2) (ops_of lca)) /\
                    Add? (snd last2))
          (ensures eq (do (concrete_merge (v_of lca) (v_of s1) (v_of s2)) last2)
  ...
  |_ -> let l' = inverse_st lca in
       let s1' = inverse_st s1 in
       let s2' = inverse_st s2 in 
       lem_l2a_last'_base  l' s1' s2' last2;
       lem_l2a_last'_base_ind lca s1 s2 last2
...
let lem_l2a_last'_s20_s1_gt0_l1_neq_help (lca s1 s2:st)
  : Lemma (requires Seq.length (ops_of s1) > Seq.length (ops_of lca) /\
...
(*let lem_l2a_last'_s20_s1_gt0_l1_neq (lca s1 s2:st) (last1 last2:op_t)
  : Lemma (requires ops_of s2 = ops_of lca /\
                    is_prefix (ops_of lca) (ops_of s1) /\
                    length (ops_of s1) > length (ops_of lca) /\
                    not (mem_id (fst last1) (ops_of lca)) /\
...
let lem_l2a_last'_s20_s1_gt0' (lca s1 s2:st) (last1 last2:op_t)
  ...
  //if get_ele last1 <> get_ele last2 then
...
let lem_l2a_last'_s20_s1_gt0 (lca s1 s2:st) (last1 last2:op_t)
  : Lemma (requires ops_of s2 = ops_of lca /\
         ...
         (ensures (eq (do (concrete_merge (v_of lca) (do (v_of s1) last1) (v_of s2)) last2)
                      (concrete_merge (v_of lca) (do (v_of s1) last1) (do (v_of s2) last2)))) = 
  if Add? (snd last1) && get_ele last1 = get_ele last2 && fst last2 > fst last1 then ()
  else 
    (lem_l2a_last'_s20_s1_gt0_l1_neq_help lca s1 s2 last1 last2)
     //lem_l2a_last'_s20_s1_gt0_l1_neq lca s1 s2 last1 last2)

let lem_l2a_last'_help' (l:log) (last2:op_t)
  : Lemma (requires distinct_ops l /\ length l > 0 /\
                    Add? (snd last2) /\
                   (let l', last1 = un_snoc l in
                    ((get_ele last1 <> get_ele last2) (*\/
...
let lem_l2a_last'_ind (lca s1 s2:st) (last2:op_t)
  : Lemma (requires length (ops_of s2) > length (ops_of lca) /\
                    Add? (snd last2) /\
                    //not (mem_id (fst last2) (ops_of lca)) /\
                    //is_prefix (ops_of lca) (ops_of s1) /\
...
let rem_add_lastop_neq_ele' (lca s1 s2:st) (last2:op_t)
  : Lemma (requires Seq.length (ops_of s1) > Seq.length (ops_of lca) /\
                    ops_of s2 == ops_of lca /\
                    //common_pre_s2_gt0 lca s1 s2 /\
                    (let _, last1 = un_snoc (ops_of s1) in
...
(*let rec lem_l2a_last'_s20 (lca s1 s2:st) (last2:op_t)
  : Lemma (requires ops_of s2 = ops_of lca /\
                    is_prefix (ops_of lca) (ops_of s1) /\
                    Add? (snd last2) /\ 
                    not (exists_triple last2 (diff (ops_of s1) (ops_of lca))) /\
          ...
          (decreases %[length (ops_of s1)]) = 
  if ops_of s1 = ops_of lca then
     lem_l2a_last'_base lca s1 s2 last2
  else 
    (let s1' = inverse_st s1 in
     rem_add_lastop_neq_ele' lca s1 s2;
     assume (not (exists_triple last2 (diff (ops_of s1') (ops_of lca))));
     lem_inverse (ops_of lca) (ops_of s1);
     lem_l2a_last'_s20 lca s1' s2 last2; admit())
     //lem_l2a_last'_s20_s1_gt0 lca s1 s2 last2)*)
...
let help (lca s1 s2:st) (last2:op_t)
  : Lemma (requires ops_of s2 = ops_of lca /\
                    is_prefix (ops_of lca) (ops_of s1) /\
                    length (ops_of s1) > length (ops_of lca) /\
                    not (mem_id (fst last2) (ops_of lca)) /\
...
let best (lca s1 s2:st) (last1 last2:op_t)
  : Lemma (requires ops_of s2 = ops_of lca /\
                    is_prefix (ops_of lca) (ops_of s1) /\
                    length (ops_of s1) > length (ops_of lca) /\
                    not (mem_id (fst last1) (ops_of lca)) /\
...
let lem_l2a_last'_s20_ind (lca s1 s2:st) (last2:op_t)
  : Lemma (requires is_prefix (ops_of lca) (ops_of s1) /\
                    ops_of s2 = ops_of lca /\
                    Add? (snd last2) /\
                    not (mem_id (fst last2) (ops_of lca)) /\
                    length (ops_of s1) > length (ops_of lca) /\
                    (let _, last1 = un_snoc (ops_of s1) in
                    //not (mem_id (fst last1) (ops_of lca)) /\ //is needed?
                    (*(get_ele last1 <> get_ele last2) \/
                     (Add? (snd last1) /\ get_ele last1 = get_ele last2 /\ fst last2 > fst last1))) /\*)
                     ~ (Add? (snd last1) /\ get_ele last1 = get_ele last2 /\ fst last1 > fst last2)) /\
                    //not (exists_triple last2 (diff (ops_of s1) (ops_of lca))) /\
...
let rec lem_l2a_last'_s20 (lca s1 s2:st) (last2:op_t)
  ...
  if ops_of s1 = ops_of lca then
    ()
  else (assume (length (ops_of s1) > length (ops_of lca));
        let s1' = inverse_st s1 in 
        help lca s1 s2 last2;
        lem_inverse (ops_of lca) (ops_of s1);
        lem_diff (ops_of s1) (ops_of lca);
        let pre1d, last1d = un_snoc (diff (ops_of s1) (ops_of lca)) in
        let _, last1 = un_snoc (ops_of s1) in
        ...
        //assert (get_ele last1d <> get_ele last2);
        assert ((diff (ops_of s1') (ops_of lca)) = pre1d); 
        help2 (diff (ops_of s1) (ops_of lca)) last2;
        assert (not (exists_triple last2 (diff (ops_of s1') (ops_of lca))));
        lem_l2a_last'_s20 lca s1' s2 last2;
        lem_l2a_last'_s20_ind lca s1 s2 last2)

let rec lem_l2a_last' (lca s1 s2:st) (last2:op_t)
  : Lemma (requires is_prefix (ops_of lca) (ops_of s1) /\
                    Add? (snd last2) /\
                    not (mem_id (fst last2) (ops_of lca)) /\
                    //length (ops_of s1) > length (ops_of lca) /\
  ...
  if ops_of s1 = ops_of lca && ops_of s2 = ops_of lca then
    lem_l2a_last'_base lca s1 s2 last2
     ...
     lem_l2a_last'_s20 lca s1 s2 last2
        ...
        let s2' = inverse_st s2 in
        lem_l2a_last' lca s1 s2' last2;
        lem_l2a_last'_ind lca s1 s2 last2)
...
let lem_l2a_last (lca s1 s2:st)
  ...
  assert (get_ele last1d <> get_ele last2);
  assert ((diff (ops_of s1') (ops_of lca)) = pre1d);
  lem_l2a_last'_help (diff (ops_of s1) (ops_of lca)) last2;
  lem_l2a_last' lca s1' s2' last1 last2*)
...
let lem_not_exists (lastop:op_t) (l:log)
  : Lemma (requires Add? (snd lastop))
          (ensures not (exists_triple lastop l) <==>
                   (~ (exists op. mem op l /\ fst op > fst lastop /\ Add? (snd op) /\ get_ele op = get_ele lastop /\
                       (let pre,suf = pre_suf l op in
                        commutative_seq op suf))))
                  (* [SMTPat (not (exists_triple lastop l))]*) = ()

let rec lem_comm_seq (op:op_t) (l:log)
  : Lemma (requires Add? (snd op))
          (ensures commutative_seq op l <==>
                   ((forall e. mem e l /\ get_ele e = get_ele op ==>
                      (Add? (snd e) /\ fst e = fst op))))
...
let rec help (lastop:op_t) (s:concrete_st) (l:log)
  : Lemma (requires (forall e. mem e l /\ Add? (snd e) /\ get_ele e == get_ele lastop ==> fst lastop >= fst e) /\
                    (forall e. S.mem e s /\ get_ele lastop == snd e ==> fst e <= fst lastop))
          (ensures (forall e. S.mem e (apply_log s l) /\ snd e == get_ele lastop ==> fst lastop >= fst e))
          (decreases length l) =
  match length l with
  |0 -> ()
  |_ -> help lastop (do s (head l)) (tail l)

let rec forall_seq f l : Tot bool (decreases length l) =
  match length l with
  |0 -> true
  |_ -> f (head l) && forall_seq f (tail l)

(*let rec lem_exists' (lastop:op_t) (s:concrete_st) (l:log)
  : Lemma (requires Add? (snd lastop) )
          (ensures exists_triple lastop l <==> (forall e. (S.mem e (apply_log s l) /\ snd e = get_ele lastop) ==> (fst lastop < fst e /\ not (S.mem e s))))
          (decreases length l) = // [SMTPat (exists_triple lastop l)]  = 
  match length l with
  |0 -> admit ()
  |1 -> ()
  |_ -> if mem (head l) l && Add? (snd (head l)) && fst (head l) > fst lastop && get_ele (head l) = get_ele lastop &&
          (let _, suf = pre_suf l (head l) in
           commutative_seq (head l) suf)
       then admit() 
       else admit() //lem_exists' lastop (do s (head l)) (tail l)

let lem_not_exists'' (lastop:op_t) (s:concrete_st) (l:log)
  : Lemma (requires Add? (snd lastop) /\ not (exists_triple lastop l))
          (ensures (forall e. S.mem e (apply_log s l) /\ snd e == get_ele lastop ==> (not (fst lastop < fst e) \/ S.mem e s))) = 
  lem_exists' lastop s l*)

(*#push-options "--z3rlimit 50"
let rec lem_not_exists'' (lastop:op_t) (s:concrete_st) (l:log)
  : Lemma (requires Add? (snd lastop) /\ not (exists_triple lastop l)) 
          (ensures (forall e. S.mem e s /\ get_ele lastop == snd e ==> fst e <= fst lastop) ==>
                   (forall e. S.mem e (apply_log s l) /\ snd e == get_ele lastop ==> fst lastop >= fst e))
          //(forall e. mem e l /\ Add? (snd e) /\ get_ele lastop == get_ele e ==> fst lastop >= fst e)) 
          (decreases length l) = 
  (*match length l with
  |0 -> admit ()
  |_ -> assert (not (exists_triple lastop (tail l)));admit();
       //assert ((forall e. S.mem e (do s (head l)) /\ get_ele lastop == snd e ==> fst e <= fst lastop)); admit();
       lem_not_exists'' lastop (do s (head l)) (tail l);*) ()*)

(*let rec lem_comm_seq (op:op_t) (l:log)
  : Lemma (requires Add? (snd op))
          (ensures commutative_seq op l ==> (forall e. mem e l /\ get_ele op = get_ele e ==>
                      ~ ((Add? (snd e) /\ fst e < fst op) \/
                         (Rem? (snd e)))) )
          (decreases length l)
          [SMTPat (commutative_seq op l)] =
...
let rec lem_commu_seq (op:op_t) (l:log)
  : Lemma (requires Add? (snd op) /\ commutative_seq op l /\ length l > 0 /\
                    (let _, lastop = un_snoc l in
                     get_ele lastop <> get_ele op))
          (ensures (let pre, suf = un_snoc l in
                    commutative_seq op pre)) 
          (decreases length l)
          [SMTPat (commutative_seq op l)] =
  match length l with
  |1 -> ()
  |_ -> lem_commu_seq op (tail l)

let lem_not_exists' (lastop:op_t) (l:log)
  : Lemma (requires Add? (snd lastop) /\ length l > 0 /\ not (exists_triple lastop l) /\
                    (let _, lastopl = un_snoc l in
                     get_ele lastopl <> get_ele lastop))
          (ensures (let pre, _ = un_snoc l in
                    not (exists_triple lastop pre))) = 
  let pre, lastopl = un_snoc l in
  lem_not_exists lastop l;
  lemma_mem_snoc pre lastopl;
  lem_last l;
  assume (~ (exists op. mem op pre /\ fst op > fst lastop /\ Add? (snd op) /\ get_ele op = get_ele lastop /\
                       (let pre,suf = pre_suf pre op in
                        commutative_seq op suf)));
  lem_not_exists lastop pre*)
  *)
