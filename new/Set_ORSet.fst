...
let prop1 (l:concrete_st) (o1 o2 o3:op_t)
  : Lemma (requires fst o1 <> fst o2 /\ fst o2 <> fst o3 /\ fst o1 <> fst o3)
          (ensures eq (concrete_merge l (do (do l o1) o2) (do l o3))
                      (concrete_merge (do l o1) (do (do l o1) o2) (concrete_merge l (do l o1) (do l o3)))) = ()

let lem_main (l:concrete_st) (o1 o2 o3:op_t)
  : Lemma (requires fst o1 <> fst o2 /\ ~ (mem_id_s (fst o3) l))
          (ensures eq (concrete_merge l (do l o1) (do (do l o2) o3)) 
                      (concrete_merge (do l o2) (concrete_merge l (do l o1) (do l o2)) (do (do l o2) o3))) = ()

let lem_merge1 (s: concrete_st) (op: op_t) 
  : Lemma (ensures (eq (concrete_merge s s (do s op)) (do s op))) = ()

//let distinct_ts (o1 o2: op_t) : bool = (not (fst o1 == fst o2))

// let lem_mergeAddRem1 (s: concrete_st) (op_add: op_t{Add? (snd op_add)}) (op_rem: op_t{Rem? (snd op_rem)}) (op: op_t)
//   : Lemma 
//       (ensures (forall x. S.mem x (concrete_merge (do s op_add) (do (do s op_rem) op_add) (do (do s op_add) op)) ==> S.mem x (do (do (do s op_rem) op_add) op)) ) = ()

// let lem_mergeAddRem2 (s: concrete_st) (op_add: op_t{Add? (snd op_add)}) (op_rem: op_t{Rem? (snd op_rem)}) (op: op_t{Add? (snd op)})
//   : Lemma (requires ( (fst op_add <> fst op) /\ (fst op_rem <> fst op) /\ (fst op_add <> fst op_rem) ) /\ ~(S.mem (fst op, get_ele op) s) ) 
//       (ensures (forall x. S.mem x (do (do (do s op_rem) op_add) op) ==> S.mem x (concrete_merge (do s op_add) (do (do s op_rem) op_add) (do (do s op_add) op))  ) ) = ()

let lem_mergeAddRem1 (s: concrete_st) (op_add: op_t{Add? (snd op_add)}) (op_rem: op_t{Rem? (snd op_rem)}) (op: op_t)
  : Lemma (requires ( (fst op_add <> fst op) /\ (fst op_rem <> fst op) /\ (fst op_add <> fst op_rem) ) 
                    /\ ~ (S.mem (fst op, get_ele op) s) ) 
          (ensures (eq (concrete_merge (do s op_add) (do (do s op_rem) op_add) (do (do s op_add) op)) (do (do (do s op_rem) op_add) op) )) = ()

let lem_mergeAddRem2 (s: concrete_st) (op_add: op_t{Add? (snd op_add)}) (op_rem: op_t{Rem? (snd op_rem)})
  : Lemma (requires   (fst op_add <> fst op_rem)   ) 
          (ensures (eq (concrete_merge (do s op_add) (do (do s op_rem) op_add) (do s op_add) ) (do (do s op_rem) op_add)  )) = ()


let lem_merge2 (s s' : concrete_st) (op op' : op_t)
  : Lemma (requires (eq (concrete_merge s s' (do s op)) (do s' op) ))
          (ensures (eq (concrete_merge (do s op') (do s' op') (do (do s op') op)) (do (do s' op') op) )) = ()


let lem_merge3 (l a b c : concrete_st)
  : Lemma 
    (requires (eq (concrete_merge l a b) c) /\ (forall (op: op_t). (eq (concrete_merge l a (do b op)) (do c op) ))  )
    (ensures (forall (op op' : op_t). (fst op <> fst op') ==> (eq (concrete_merge l a (do (do b op) op')) (do (do c op) op')) )) = ()


#push-options "--z3rlimit 50"

// let lem_merge4 (s s' : concrete_st) (op: op_t{Add? (snd op)}) (op': op_t{Rem? (snd op')}) (op'' : op_t)
//   : Lemma (requires (eq (concrete_merge (do s op) (do s' op) (do (do s op) op'')) (do (do s' op) op'') )
//           /\ (forall (st : concrete_st). (eq (concrete_merge (do st op) (do (do st op') op) (do st op'')) (do (do (do st op') op) op'') ))
//           )
//     (ensures (eq (concrete_merge (do s op) (do (do s' op') op) (do (do s op) op'')) (do (do (do s' op') op) op'') )) = ()


let lem_merge4 (s s' : concrete_st) (op: op_t{Add? (snd op)}) (op': op_t{Rem? (snd op')})
...
let lem_merge5 (s s' : concrete_st) (op: op_t{Add? (snd op)}) (op': op_t{Rem? (snd op') /\ get_ele op' == get_ele op}) (op'': op_t) (op1 : op_t{get_ele op1 == get_ele op})
  : Lemma (requires (eq (concrete_merge (do s op) (do (do s' op') op) (do (do s op) op'')) (do (do (do s' op') op) op'') ))
          (ensures (eq (concrete_merge (do s op) (do (do (do s' op1) op') op) (do (do s op) op'')) (do (do (do (do s' op1) op') op) op'') )) = ()

let lem2_notes (s s':concrete_st) (o o'':op_t)
  : Lemma (requires eq (concrete_merge s s' (do s o'')) (do s' o''))
          (ensures eq (concrete_merge (do s o) (do s' o) (do (do s o) o'')) (do (do s' o) o'')) = ()

#set-options "--query_stats"
#push-options "--z3rlimit 100"
// s -> sigma' (l) /\ s' = o' (l)
let lem1 (s s':concrete_st) (o o' o'':op_t)
  : Lemma (requires eq (concrete_merge s' (do s' o'') (do s o')) (do (do s o') o''))
                    //Rem? (snd o) /\ //~ (mem_id_s (fst o'') s') /\
                    
          (ensures ((forall l. (eq (concrete_merge l (do l o'') (do l o)) (do (do l o'') o))) ==>
                       eq (concrete_merge s' (do s' o'') (do (do s o) o')) (do (do (do s o) o') o'')) \/
                 
                   ((forall l. (eq (concrete_merge l (do l o'') (do l o)) (do (do l o) o''))) ==>
                       eq (concrete_merge s' (do s' o'') (do (do s o) o')) (do (do (do s o) o') o''))) = ()

let lem2 (l s s':concrete_st) (o o' o'':op_t)
  : Lemma (requires eq (concrete_merge (do l o'') s' (do (do l o') o'')) s)
          (ensures ((forall a. eq (concrete_merge a (do a o) (do a o')) (do (do a o') o)) ==>
                       eq (concrete_merge (do l o'') (do s' o) (do (do l o') o'')) (do s o)) \/
                   ((forall a. eq (concrete_merge a (do a o) (do a o')) (do (do a o) o')) ==>
                       eq (concrete_merge (do l o'') (do s' o) (do (do l o') o'')) (do s o))) = ()

let prop5 (l s s1 s2 s3:concrete_st) (o2 o3:op_t)
  : Lemma (requires eq (concrete_merge s s1 s3) s2 /\ fst o2 <> fst o3 /\
                    eq (concrete_merge l s (do l o3)) s3 /\
                    eq (concrete_merge l s1 (do l o3)) s2 /\
                    Rem? (snd o3) /\ Add? (snd o2) /\ get_ele o2 = get_ele o3)
          (ensures eq (concrete_merge s (do s1 o2) s3) (do s2 o2)) = ()

let prop4 (l s:concrete_st) (o1 o2 o3:op_t)
  : Lemma (requires eq (concrete_merge l (do (do l o1) o2) s) (do (do s o1) o2) /\
                    //eq (concrete_merge l (do l o1) s) (do s o1) /\
                    //eq (concrete_merge l (do (do l o1) o2) s) 
                       //(concrete_merge (do l o1) (do (do l o1) o2) (concrete_merge l (do l o1) s)) /\
                    //fst o1 <> fst o2 /\ fst o2 <> fst o3 /\ fst o1 <> fst o3 /\
                    //~ (mem_id_s (fst o1) s) /\
                    (*~ (mem_id_s (fst o3) s) /\ ~ (mem_ele (get_ele o2) s) /\*) //~ (mem_ele (get_ele o1) s) /\
                    Add? (snd o1) /\ Add? (snd o2) /\ Rem? (snd o3) /\ get_ele o3 = get_ele o1 /\ get_ele o3 = get_ele o2 /\
                    (forall a. eq (concrete_merge a (do a o2) (do a o3)) (do (do a o2) o3) \/
                          eq (concrete_merge a (do a o2) (do a o3)) (do (do a o3) o2)))   
          (ensures eq (concrete_merge l (do (do l o1) o2) (do s o3)) (do (do (do s o3) o1) o2)) = ()

let prop5 (l s:concrete_st) (o1 o2 o3:op_t)
  : Lemma (requires eq (concrete_merge l (do (do l o1) o2) s) 
                       (concrete_merge (do l o1) (do (do l o1) o2) (concrete_merge l (do l o1) s)) /\
                    fst o1 <> fst o2 /\ fst o1 <> fst o3 /\ fst o2 <> fst o3)
          (ensures eq (concrete_merge l (do (do l o1) o2) (do s o3)) 
                       (concrete_merge (do l o1) (do (do l o1) o2) (concrete_merge l (do l o1) (do s o3)))) = admit()

let lem2' (l a b:concrete_st) (o o' o'':op_t)
  : Lemma (requires eq (concrete_merge (do l o'') b a) (concrete_merge l b (do l o')) /\
                    eq (concrete_merge l (do l o'') (do l o')) a /\
                    fst o <> fst o' /\ fst o' <> fst o'' /\ fst o <> fst o'' /\
                    ~ (mem_id_s (fst o'') l))
          (ensures eq (concrete_merge (do l o'') (do b o) a) (concrete_merge l (do b o) (do l o'))) = ()



let trial (l s s':concrete_st) (o o' o'':op_t)
  : Lemma (requires eq (concrete_merge (do l o'') (do (do l o') o'') s) s' /\
                    (*eq (concrete_merge l s (do l o')) s' /\*) //Add? (snd o) /\ //Rem? (snd o') /\
                    //s = (do l o'') /\ s' = (do (do l o') o'')  /\
                    //eq (concrete_merge l (do l o'') (do l o')) (do (do l o') o'') /\
                     ~ (mem_id_s (fst o) l) (*/\ ~ (mem_id_s (fst o') s'*)) 
          (ensures eq (concrete_merge (do l o'') (do (do l o') o'') (do s o)) (do s' o)) = ()


let lem2' (l s s' c:concrete_st) (o o'':op_t)
  : Lemma (requires eq (concrete_merge (do l o'') s' s) c /\ 
                    ~ (mem_id_s (fst o'') l) /\ ~ (mem_id_s (fst o) l) /\ 
                    ~ (mem_id_s (fst o) s') /\ ~ (mem_id_s (fst o'') s') /\
                    ~ (mem_id_s (fst o) s) /\ ~ (mem_id_s (fst o'') s) /\
                    ~ (mem_id_s (fst o) c) /\ ~ (mem_id_s (fst o'') c) /\ Add? (snd o''))
          (ensures eq (concrete_merge (do l o'') (do s' o) s) (do c o)) = admit()
