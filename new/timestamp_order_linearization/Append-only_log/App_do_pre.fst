...
(*let rec fts_merge_pre1 (lca s1 s2:st)
  : Lemma (requires consistent_branches lca s1 s2)
          (ensures merge_pre (v_of lca) (v_of s1) (v_of s2))
          (decreases %[length (ops_of s1); length (ops_of s2); length (ops_of lca)]) =
  if ops_of s1 = ops_of s2 then
    if ops_of s2 = ops_of lca then ()
    else 
      (let s2' = inverse_st s2 in
       assume (consistent_branches lca s1 s2');
       fts_merge_pre1 lca s1 s2')
  else admit()


//#push-options "--z3rlimit 100"
let rec fts_merge_pre1 (lca s1' s2:st) (last1:op_t)
  ...
  if ops_of s1' = ops_of lca then ()
    ...
    (let pre, lastop = un_snoc (ops_of s1') in
     ...
     //assert (consistent_branches lca s1'' s2 ); admit();
     assert (//do_pre (v_of s1'') last1 /\
             //consistent_branches lca (do_st s1'' last1) s2 /\
             //consistent_branches lca s1'' s2 /\
             merge_pre (v_of lca) (do (v_of s1'') last1) (v_of s2)); admit();
     fts_merge_pre1 lca s1'' s2 last1)*)
