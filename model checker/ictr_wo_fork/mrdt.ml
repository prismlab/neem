...
type config = {
  ...
  ns : int;                (* ns is the number of steps in an execution *)
...
let init_config = {i = initI; ns = initNs; r = initR; n = initN; h = initH; l = initL; g = initG; vis = initVis}
...
let apply (c:config) (srcRid:repId) (o:event) : config = 
  ...
  debug_print "\n****Source ver of apply is (%d,%d)" (fst srcVer) (snd srcVer);
  let newI = gen_cid () in
  let newNs = c.ns + 1 in
  ...
  let newVis = List.fold_left (fun acc o1 -> VisSet.add (o1,o) acc) c.vis (c.l srcVer) in
  let new_c = {i = newI; ns = newNs; r = newR; n = newN; h = newH; l = newL; g = newG; vis = newVis} in
  print_dag new_c;
...
let merge (c:config) (r1:repId) (r2:repId) : config =
  ...
  let newG = let e = add_edge c.g (c.h r1) (Merge (r1, r2)) newVer in
              add_edge e (c.h r2) (Merge (r1, r2)) newVer in
  let new_c = {i = newI; ns = newNs; r = c.r; n = newN; h = newH; l = newL; g = newG; vis = c.vis} in
  print_dag new_c;
...
let apply (c:config) (srcRid:repId) (o:event) : config = 
  ...
  let newG = add_edge c.g srcVer (Apply o) newVer in
  let newVis = List.fold_left (fun acc o1 -> VisSet.add (o1,o) acc) c.vis (c.l srcVer) in
  let new_c = {i = newI; r = newR; n = newN; h = newH; l = newL; g = newG; vis = newVis} in
  print_dag new_c;
  (*debug_print "\nLinearization check for apply started...";*)
  assert (lin_check srcRid new_c); 
  (*assert (sim_check srcRid new_c);*)
