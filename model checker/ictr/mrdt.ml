...
let config_eq c1 c2 =
  RepSet.equal c1.r c2.r &&
  ver_eq (domain_v c1.n) (domain_v c2.n) &&
  (*VerSet.equal (domain_v c1.n) (domain_v c2.n) &&*)
  VerSet.for_all (fun v -> eq (VerMap.find v c1.n) (VerMap.find v c2.n)) (domain_v c1.n) (*&&
  RepSet.equal (domain_r c1.h) (domain_r c2.h) &&
  RepSet.for_all (fun v -> (RepIdMap.find v c1.h) = (RepIdMap.find v c2.h)) (domain_r c1.h) &&
  RepSet.equal (domain_r c1.v) (domain_r c2.v) &&
  RepSet.for_all (fun v -> VerSet.equal (RepIdMap.find v c1.v) (RepIdMap.find v c2.v)) (domain_r c1.v) &&
  VerSet.equal (domain_v c1.l) (domain_v c2.l) &&
  VerSet.for_all (fun v -> (VerMap.find v c1.l) = (VerMap.find v c2.l)) (domain_v c1.l) &&
  VerSet.equal (c1.g.vertices) (c2.g.vertices) &&
  c1.g.edges = c2.g.edges &&
  VisSet.equal (c1.vis) (c2.vis)*)
...
let init_config = {i = initI; r = initR; n = initN; h = initH; v = initV; l = initL; g = initG; vis = initVis}
...
let string_of_event ((_, r, _):event) =
  Printf.sprintf "(r%d, %s)" r "Incr"
...
let createBranch (c:config) (srcRid:repId) (dstRid:repId) : config =
  ...
  let new_c = {i = newI; r = newR; n = newN; h = newH; v = newV; l = newL; g = newG; vis = c.vis} in
  print_dag new_c;
...
let apply (c:config) (srcRid:repId) (o:event) : config = 
  ...
  let new_c = {i = newI; r = c.r; n = newN; h = newH; v = newV; l = newL; g = newG; vis = newVis} in
  print_dag new_c;
...
let merge (c:config) (r1:repId) (r2:repId) : config =
  ...
  let new_c = {i = newI; r = c.r; n = newN; h = newH; v = newV; l = newL; g = newG; vis = c.vis} in
  print_dag new_c;
...
3. If both lists are non-empty, 
    ...
    if rc e1 e2 = Fst_then_snd, then e2::(linearize l1 (tail l2)
        ...
        i. find if there exists an e3 in (l2\fe2) such that (rc e1 e3 = Fst_then_snd &&
