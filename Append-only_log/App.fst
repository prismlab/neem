...
let concrete_do_pre (s:concrete_st) (op:log_entry) = 
  not (mem_id_s (fst op) s) /\ 
  forallb (fun e -> lt (fst e) (fst op)) s
...
let eq (st_s:concrete_st_s) (st:concrete_st) = admit()

//initial states are equivalent
let initial_eq _
  : Lemma (ensures eq init_st_s init_st) = admit()
...
let rec lem_foldl (s:concrete_st) (l:log)
  : Lemma (requires foldl_prop s l)
          (ensures (length l = 0 ==> (seq_foldl s l = s)) /\
                   (forall id id1. length l > 0 /\ mem_id id l /\ mem_id_s id1 s ==> lt id1 id) /\
                   unique_s (seq_foldl s l) /\ total_order (seq_foldl s l))
          (decreases length l)
  = match length l with
    |0 -> ()
