...
let rc_inter_right' (l s1 s2 s3:concrete_st) (o1 o2 ob ol o:op_t)
  : Lemma (requires distinct_ops o1 o2 /\ Fst_then_snd? (rc o1 o2) /\  
                    ...
                    (forall k. eq_l (fst (sel_k (merge (do l ol) (do (do s1 ol) o1) (do (do (do (do s2 o) ob) ol) o2)) k))
                               (fst (sel_k (do (do (do (do (do s3 o) ob) ol) o1) o2) k)) /\
                          eq_e (snd (sel_k (merge (do l ol) (do (do s1 ol) o1) (do (do (do (do s2 o) ob) ol) o2)) k))
                               (snd (sel_k (do (do (do (do (do s3 o) ob) ol) o1) o2) k))))
...
let rc_inter_left' (l s1 s2 s3:concrete_st) (o1 o2 ob ol o:op_t)
  : Lemma (requires distinct_ops o1 o2 /\ Fst_then_snd? (rc o1 o2) /\  
                    ...
                    (forall k. eq_l (fst (sel_k (merge (do l ol) (do (do (do (do s1 o) ob) ol) o1) (do (do s2 ol) o2)) k))
                               (fst (sel_k (do (do (do (do (do s3 o) ob) ol) o1) o2) k)) /\
                          eq_e (snd (sel_k (merge (do l ol) (do (do (do (do s1 o) ob) ol) o1) (do (do s2 ol) o2)) k))
                               (snd (sel_k (do (do (do (do (do s3 o) ob) ol) o1) o2) k))))
...
let rc_inter_lca' (l s1 s2 s3:concrete_st) (o1 o2 ol oi:op_t)
  : Lemma (requires distinct_ops o1 o2 /\ Fst_then_snd? (rc o1 o2) /\ 
                    ...
                    (forall k. eq_l (fst (sel_k (merge (do (do l oi) ol) (do (do (do s1 oi) ol) o1) (do (do (do s2 oi) ol) o2)) k))
                               (fst (sel_k (do (do (do (do s3 oi) ol) o1) o2) k)) /\
                          eq_e (snd (sel_k (merge (do (do l oi) ol) (do (do (do s1 oi) ol) o1) (do (do (do s2 oi) ol) o2)) k))
                               (snd (sel_k (do (do (do (do s3 oi) ol) o1) o2) k))))
...
let ts_ind_lca' (l:concrete_st) (ol o1 o2:op_t)
  : Lemma (requires distinct_ops o1 o2 /\ Ts_order? (rc o1 o2) /\ distinct_ops ol o1 /\ distinct_ops ol o2 /\ fst o1 < fst o2 /\
                    eq (merge l (do l o1) (do l o2)) (do (do l o1) o2) /\
                    (forall k. eq_l (fst (sel_k (merge (do l ol) (do (do l ol) o1) (do (do l ol) o2)) k))
                               (fst (sel_k (do (do (do l ol) o1) o2) k)) /\
                          eq_e (snd (sel_k (merge (do l ol) (do (do l ol) o1) (do (do l ol) o2)) k))
                               (snd (sel_k (do (do (do l ol) o1) o2) k))))
...
let ts_inter_base_right' (l s1 s2 s3:concrete_st) (ob ol o1 o2:op_t) 
  : Lemma (requires distinct_ops ob ol /\ (Fst_then_snd? (rc ob ol) \/ (Ts_order? (rc ob ol) /\ fst ob < fst ol)) /\ 
                    ...
                    (forall k. eq_l (fst (sel_k (merge (do l ol) (do (do s1 ol) o1) (do (do (do s2 ob) ol) o2)) k))
                               (fst (sel_k (do (do (do (do s3 ob) ol) o1) o2) k)) /\
                          eq_e (snd (sel_k (merge (do l ol) (do (do s1 ol) o1) (do (do (do s2 ob) ol) o2)) k))
                               (snd (sel_k (do (do (do (do s3 ob) ol) o1) o2) k))))
...
let ts_inter_base_left' (l s1 s2 s3:concrete_st) (ob ol o1 o2:op_t) 
  : Lemma (requires distinct_ops ob ol /\ (Fst_then_snd? (rc ob ol) \/ (Ts_order? (rc ob ol) /\ fst ob < fst ol)) /\ 
                    ...
                    (forall k. eq_l (fst (sel_k (merge (do l ol) (do (do (do s1 ob) ol) o1) (do (do s2 ol) o2)) k))
                               (fst (sel_k (do (do (do (do s3 ob) ol) o1) o2) k)) /\
                          eq_e (snd (sel_k (merge (do l ol) (do (do (do s1 ob) ol) o1) (do (do s2 ol) o2)) k))
                               (snd (sel_k (do (do (do (do s3 ob) ol) o1) o2) k))))
...
let one_op_inter_base_right' (l s1 s2 s3:concrete_st) (ob ol o1:op_t) 
  : Lemma (requires distinct_ops ob ol /\ (Fst_then_snd? (rc ob ol) \/ (Ts_order? (rc ob ol) /\ fst ob < fst ol)) /\ 
                    ...
                    eq (merge l (do s1 ol) (do s2 ob)) (do (do s3 ob) ol) /\ //***EXTRA***
                    (forall k. eq_l (fst (sel_k (merge (do l ol) (do s1 ol) (do (do (do s2 ob) ol) o1)) k))
                               (fst (sel_k (do (do (do s3 ob) ol) o1) k)) /\
                          eq_e (snd (sel_k (merge (do l ol) (do s1 ol) (do (do (do s2 ob) ol) o1))k))
                               (snd (sel_k (do (do (do s3 ob) ol) o1) k))))
...
let one_op_inter_base_left' (l s1 s2 s3:concrete_st) (ob ol o1:op_t) 
  : Lemma (requires distinct_ops ob ol /\ (Fst_then_snd? (rc ob ol) \/ (Ts_order? (rc ob ol) /\ fst ob < fst ol)) /\ 
                    ...
                    (forall k. eq_l (fst (sel_k (merge (do l ol) (do (do s1 ob) ol) (do (do s2 ol) o1)) k))
                               (fst (sel_k (do (do (do s3 ob) ol) o1) k)) /\
                          eq_e (snd (sel_k (merge (do l ol) (do (do s1 ob) ol) (do (do s2 ol) o1)) k))
                               (snd (sel_k (do (do (do s3 ob) ol) o1) k))))
...
let one_op_inter_right' (l s1 s2 s3:concrete_st) (o1 ob ol o:op_t)
  : Lemma (requires distinct_ops ob ol /\ (Fst_then_snd? (rc ob ol) \/ (Ts_order? (rc ob ol) /\ fst ob < fst ol)) /\ 
                    ...
                    (forall k. eq_l (fst (sel_k (merge (do l ol) (do (do s1 ol) o1) (do (do (do s2 o) ob) ol)) k))
                               (fst (sel_k (do (do (do (do s3 o) ob) ol) o1) k)) /\
                          eq_e (snd (sel_k (merge (do l ol) (do (do s1 ol) o1) (do (do (do s2 o) ob) ol)) k))
                               (snd (sel_k (do (do (do (do s3 o) ob) ol) o1) k))))
...
let one_op_inter_left' (l s1 s2 s3:concrete_st) (o1 ob ol o:op_t)
  : Lemma (requires distinct_ops ob ol /\ (Fst_then_snd? (rc ob ol) \/ (Ts_order? (rc ob ol) /\ fst ob < fst ol)) /\ 
                    ...
                    (forall k. eq_l (fst (sel_k (merge (do l ol) (do (do (do s1 o) ob) ol) (do (do s2 ol) o1)) k))
                               (fst (sel_k (do (do (do (do s3 o) ob) ol) o1) k)) /\
                          eq_e (snd (sel_k (merge (do l ol) (do (do (do s1 o) ob) ol) (do (do s2 ol) o1)) k))
                               (snd (sel_k (do (do (do (do s3 o) ob) ol) o1) k)))) 
...
let one_op_inter_lca' (l s1 s2 s3:concrete_st) (o1 ol oi:op_t)
  : Lemma (requires (exists o. Fst_then_snd? (rc o ol) \/ (Ts_order? (rc o ol) /\ fst o < fst ol)) /\ 
                    ...
                    eq (merge (do l ol) (do s1 ol) (do (do s2 ol) o1)) (do (do s3 ol) o1) /\
                    (forall k. eq_l (fst (sel_k (merge (do (do l oi) ol) (do (do s1 oi) ol) (do (do (do s2 oi) ol) o1)) k))
                               (fst (sel_k (do (do (do s3 oi) ol) o1) k)) /\
                          eq_e (snd (sel_k (merge (do (do l oi) ol) (do (do s1 oi) ol) (do (do (do s2 oi) ol) o1)) k))
                               (snd (sel_k (do (do (do s3 oi) ol) o1) k)))) 
...
let zero_op_inter_base_right' (l s1 s2 s3:concrete_st) (ob ol:op_t) 
  : Lemma (requires distinct_ops ob ol /\ (Fst_then_snd? (rc ob ol) \/ (Ts_order? (rc ob ol) /\ fst ob < fst ol)) /\ 
                    ...
                    eq (merge l (do s1 ol) (do s2 ob)) (do (do s3 ob) ol) /\ //***EXTRA***
                    (forall k. eq_l (fst (sel_k (merge (do l ol) (do s1 ol) (do (do s2 ob) ol)) k))
                               (fst (sel_k (do (do s3 ob) ol) k)) /\
                          eq_e (snd (sel_k (merge (do l ol) (do s1 ol) (do (do s2 ob) ol)) k))
                               (snd (sel_k (do (do s3 ob) ol) k)))) 
...
let zero_op_inter_base_left' (l s1 s2 s3:concrete_st) (ob ol:op_t) 
  : Lemma (requires distinct_ops ob ol /\ (Fst_then_snd? (rc ob ol) \/ (Ts_order? (rc ob ol) /\ fst ob < fst ol)) /\ 
                    ...
                    eq (merge (do l ol) (do s1 ol) (do s2 ol)) (do s3 ol) /\
                    eq (merge l (do s1 ob) (do s2 ol)) (do (do s3 ob) ol) /\ //***EXTRA***
                    (forall k. eq_l (fst (sel_k (merge (do l ol) (do (do s1 ob) ol) (do s2 ol)) k))
                               (fst (sel_k (do (do s3 ob) ol) k)) /\
                          eq_e (snd (sel_k (merge (do l ol) (do (do s1 ob) ol) (do s2 ol)) k))
                               (snd (sel_k (do (do s3 ob) ol) k))))
...
let zero_op_inter_right' (l s1 s2 s3:concrete_st) (ob ol o:op_t)
  : Lemma (requires distinct_ops ob ol /\ (Fst_then_snd? (rc ob ol) \/ (Ts_order? (rc ob ol) /\ fst ob < fst ol)) /\ 
                    ...
                    (forall k. eq_l (fst (sel_k (merge (do l ol) (do s1 ol) (do (do (do s2 o) ob) ol)) k))
                               (fst (sel_k (do (do (do s3 o) ob) ol) k)) /\
                          eq_e (snd (sel_k (merge (do l ol) (do s1 ol) (do (do (do s2 o) ob) ol)) k))
                               (snd (sel_k (do (do (do s3 o) ob) ol) k)))) 
...
let zero_op_inter_left' (l s1 s2 s3:concrete_st) (ob ol o:op_t)
  : Lemma (requires distinct_ops ob ol /\ (Fst_then_snd? (rc ob ol) \/ (Ts_order? (rc ob ol) /\ fst ob < fst ol)) /\ 
                    ...
                    (forall k. eq_l (fst (sel_k (merge (do l ol) (do (do (do s1 o) ob) ol) (do s2 ol)) k))
                               (fst (sel_k (do (do (do s3 o) ob) ol) k)) /\
                          eq_e (snd (sel_k (merge (do l ol) (do (do (do s1 o) ob) ol) (do s2 ol)) k))
                               (snd (sel_k (do (do (do s3 o) ob) ol) k))))
...
let comm_inter_lca' (l s1 s2 s3:concrete_st) (o1 o2 ol:op_t)
  : Lemma (requires distinct_ops o1 o2 /\ Either? (rc o1 o2) /\ distinct_ops ol o1 /\ distinct_ops ol o2 /\
                    (exists o'. Fst_then_snd? (rc o' ol) \/ (Ts_order? (rc o' ol) /\ fst o' < fst ol)) /\
                    eq (merge l (do s1 o1) (do s2 o2)) (do (do s3 o1) o2) /\
                    (forall k. eq_l (fst (sel_k (merge (do l ol) (do (do s1 ol) o1) (do (do s2 ol) o2)) k))
                               (fst (sel_k (do (do (do s3 ol) o1) o2) k)) /\
                          eq_e (snd (sel_k (merge (do l ol) (do (do s1 ol) o1) (do (do s2 ol) o2)) k))
                               (snd (sel_k (do (do (do s3 ol) o1) o2) k))))
