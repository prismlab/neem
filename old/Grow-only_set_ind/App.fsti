...
let rec linearizable_gt0_s1' (lca s1 s2:st) (last1:log_entry)
  : Lemma (requires is_prefix (ops_of lca) (ops_of s1) /\
                    is_prefix (ops_of lca) (ops_of s2) /\
                    (forall id id1. mem_id id (ops_of lca) /\ mem_id id1 (diff (ops_of s1) (ops_of lca)) ==> lt id id1) /\
                    (forall id id1. mem_id id (ops_of lca) /\ mem_id id1 (diff (ops_of s2) (ops_of lca)) ==> lt id id1) /\
          ...
          (decreases length (ops_of s1)) =
  if ops_of s1 = ops_of lca && ops_of s2 = ops_of lca then
    admit() //linearizable_gt0_s1'_base lca s1 s2 last1
  else if ops_of s2 = ops_of lca then
    admit() //linearizable_gt0_s1'_s20 lca s1 s2 last1
        ...
        inverse_diff_id_s2' (ops_of lca) (ops_of s1) (ops_of s2);
        lem_diff (ops_of s2) (ops_of lca);
        lem_un_snoc_id (diff (ops_of s2') (ops_of lca)) (diff (ops_of s2) (ops_of lca));*)
        assume (is_prefix (ops_of lca) (ops_of s1) /\
                ...
                (forall id. mem_id id (ops_of lca) ==> lt id (fst last1)) /\
                not (mem_id (fst last1) (diff (ops_of s2') (ops_of lca))));
        linearizable_gt0_s1' lca s1 s2' last1;
        linearizable_gt0_s1'_s2_gt0_ind lca s1 s2 last1)
#pop-options
...
let linearizable_gt0_s1 (lca s1 s2:st)
  ...
  linearizable_gt0_s1' lca (inverse_st s1) s2 last1
