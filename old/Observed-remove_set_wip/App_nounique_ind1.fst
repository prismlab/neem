...
let lem_l2a_l1r_eq''_base_pre (lca s1 s2:st) (last1 last2:log_entry)
...
let rec lem_l2a_l1r_eq''_base (lca s1 s2:st) (last1 last2:log_entry)
  ...
  |_ -> let l' = inverse_st lca in
       let s1' = inverse_st s1 in
       let s2' = inverse_st s2 in 
       lem_l2a_l1r_eq''_base_pre lca s1 s2 last1 last2;
...
let lem_l2a_l1r_eq' (lca s1 s2:st)
 : Lemma (requires length (ops_of s2) > 0 /\
                   length (ops_of s1) > 0 /\
   ...
   let _, last1 = un_snoc (ops_of s1) in
   let s1' = inverse_st s1 in
   assert (v_of s1 = do (v_of s1') last1); 
   lem_l2a_l1r_eq'' lca s1' s2' last1 last2
...
let base (lca s1 s2:st) (last1 last2:log_entry)
  : Lemma (requires ops_of s1 = ops_of lca /\ ops_of s2 = ops_of lca /\
                    Add? (snd last2) /\ Rem? (snd last1) /\ get_ele last1 = get_ele last2 /\ 
                    not (mem_id (fst last1) (ops_of lca)) /\
                    not (mem_id (fst last2) (ops_of lca)))
...
let test (lca s1 s2:st) (last1 last2:log_entry)
  : Lemma (requires common_pre_nc lca s1 s2 /\
                    ops_of s1 = ops_of lca /\
                    length (ops_of s2) > length (ops_of lca) /\
                    Add? (snd last2) /\ Rem? (snd last1) /\ get_ele last1 = get_ele last2 /\
