...
let rec linearizable_gt0_inv2_c2'_s1_0 (lca s1 s2:st)
 : Lemma (requires common_pre1 lca s1 s2 /\ 
                   ops_of s1 = ops_of lca /\
                   (let _, last2 = un_snoc (ops_of s2) in
                    Add? (snd last2) /\
   ...
   let s2' = inverse_st s2 in
   if length (diff (ops_of s2) (ops_of lca)) = 1
     then base_case lca s1 s2
   else (lem_len_diff (ops_of s2) (ops_of lca);
         assume (length (diff (ops_of s2) (ops_of lca)) > 1); 
         assume (length (ops_of s2) > 1);
         let s2' = inverse_st s2 in
         let s2'' = inverse_st s2' in
         let _, last2 = un_snoc (ops_of s2) in
         assume (concrete_do_pre (v_of s2'') last2);
         assume (valid_st ((do (v_of s2'') last2), hide (snoc (ops_of s2'') last2))); 
         assume (common_pre1 lca s1 ((do (v_of s2'') last2), hide (snoc (ops_of s2'') last2)));
         assume (length (snoc (ops_of s2'') last2) > 0);
         assume (let _, last2_new = un_snoc (snoc (ops_of s2'') last2) in
                 Add? (snd last2_new) /\
                 not (exists_triple last2_new (diff (ops_of s1) (ops_of lca))) /\
                 (let s2'_new = inverse_st ((do (v_of s2'') last2), hide (snoc (ops_of s2'') last2)) in
                 is_prefix (ops_of lca) (ops_of s2'_new) /\
                 concrete_merge_pre (v_of lca) (v_of s1) (v_of s2'_new) /\
                 concrete_do_pre (concrete_merge (v_of lca) (v_of s1) (v_of s2'_new)) last2_new));
         
         linearizable_gt0_inv2_c2'_s1_0 lca s1 ((do (v_of s2'') last2), hide (snoc (ops_of s2'') last2)); admit();
...
let rec linearizable_gt0_inv2_c2' (lca s1 s2:st)
 : Lemma (requires common_pre1 lca s1 s2 /\ 
   ...
   let s2' = inverse_st s2 in
   if ops_of s1 = ops_of lca then
     linearizable_gt0_inv2_c2'_s1_0 lca s1 s2          
   else 
...
#push-options "--z3rlimit 1200 --fuel 1 --ifuel 1"
let rec linearizable_gt0_inv2_c2' (lca s1 s2:st)
 : Lemma (requires common_pre1 lca s1 s2 /\ 
                   (let _, last2 = un_snoc (ops_of s2) in
                    Add? (snd last2) /\
