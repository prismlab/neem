...
let rec lem_seq_foldl (x:concrete_st) (l:log)
  : Lemma (requires foldl_prop x l)
          (ensures (length l > 0 ==> foldl_prop x (fst (un_snoc l)) /\ 
                   ...
                   (length l > 0 /\ concrete_do_pre x (head l) /\ foldl_prop (do x (head l)) (tail l) ==>
                     foldl_prop x l))
          (decreases length l) =
  match length l with
  |0 -> ()
...
let rec lem_seq_foldl_forall (l:log) 
  : Lemma (requires true)
          (ensures (forall x. (foldl_prop x l ==>
                   (length l > 0 ==> (foldl_prop x (fst (un_snoc l)) /\ 
                   concrete_do_pre (seq_foldl x (fst (un_snoc l))) (last l) /\
                   foldl_prop (do x (head l)) (tail l) /\
                   (seq_foldl x l = seq_foldl (do x (head l)) (tail l)) /\
                   (seq_foldl x l = do (seq_foldl x (fst (un_snoc l))) (last l)))) /\
...
let rec split_prefix_forall (l:log) (a:log)
  : Lemma (requires is_prefix l a)
          (ensures (forall s. foldl_prop s a ==> foldl_prop s l /\ foldl_prop (seq_foldl s l) (diff a l) /\
                   (seq_foldl s a = seq_foldl (seq_foldl s l) (diff a l)) /\
                   (forall e. mem e l ==> mem e a) /\
                   (Seq.length a > Seq.length l ==> (last a) = (last (diff a l)))))
          (decreases Seq.length l)
...
val lem_foldl_prop_comm_41 (x:concrete_st) (l:log) (op:log_entry)
  : Lemma (requires commutative_seq op l /\ distinct_ops l /\ length l > 0 /\
                    (let hd = head l in let tl = tail l in
...
let lem_foldl_prop_comm_42 (x:concrete_st) (l:log) (op:log_entry)
  : Lemma (requires commutative_seq op l /\ distinct_ops l /\ length l > 0 /\
                    (let hd = head l in let tl = tail l in
                      (foldl_prop x (cons op l) /\ foldl_prop x (snoc l op))))
          (ensures (let hd = head l in let tl = tail l in
          ...
          [SMTPat (commutative_seq op l /\ distinct_ops l /\ length l > 0)] =
  lem_foldl_prop_comm_41 x l op;
  let hd = head l in let tl = tail l in
  assert (foldl_prop x (cons hd (cons op tl))); 
  lem_seq_foldl x (cons hd (cons op tl));
  lemma_mem_append (create 1 hd) (cons op tl);
  lemma_tl hd (cons op tl);
  assert (hd = head (cons hd (cons op tl))); 
  assert (cons op tl = tail (cons hd (cons op tl)));
  assert (foldl_prop (do x hd) (cons op tl));
  lem_seq_foldl (do x hd) (cons op tl);
  assert (length (cons op tl) > 0);
  lemma_tl op tl;
  assert (do (do x hd) op = seq_foldl x (cons hd (create 1 op)));
  ()
...
val lem_eq_is_equiv_c3 (l:log) (op:log_entry)
  : Lemma (requires commutative_seq op l /\ distinct_ops l /\ length l > 0 /\
                    (let hd = head l in let tl = tail l in
                    (forall x. foldl_prop x (cons op l) /\ foldl_prop x (snoc l op) ==>
                          (lem_foldl_prop_comm l op;
...
val lem_eq_is_equiv_c4 (l:log) (op:log_entry)
  : Lemma (requires commutative_seq op l /\ distinct_ops l /\ length l > 0 /\
                    (let hd = head l in let tl = tail l in
                    (forall x. foldl_prop x (cons op l) /\ foldl_prop x (snoc l op) ==>
                          (lem_foldl_prop_comm l op;
...
val lem_case1_eq (l:log) (op:log_entry)
  : Lemma (requires commutative_seq op l /\ distinct_ops l /\ length l > 0 /\
                    (let hd = head l in let tl = tail l in
                    lem_foldl_prop_comm l op;
                    (forall x. foldl_prop x (cons op l) /\ foldl_prop x (snoc l op) ==>             
...
val lem_case3_eq  (l:log) (op:log_entry)
  : Lemma (requires commutative_seq op l /\ distinct_ops l /\ length l > 0 /\
                   (let hd = head l in let tl = tail l in
                   (forall x. foldl_prop x (cons op l) /\ foldl_prop x (snoc l op) ==>
                         (lem_foldl_prop_comm l op;
...
val lem_case4_eq  (l:log) (op:log_entry)
  : Lemma (requires commutative_seq op l /\ distinct_ops l /\ length l > 0 /\
                   (let hd = head l in let tl = tail l in
                   (forall x. foldl_prop x (cons op l) /\ foldl_prop x (snoc l op) ==>
                         (lem_foldl_prop_comm l op;
...
let rec lem_foldl_comm (l:log) (op:log_entry)
  : Lemma (requires commutative_seq op l /\ distinct_ops l)
          (ensures (forall x. foldl_prop x (cons op l) /\ foldl_prop x (snoc l op) ==>
                         eq (seq_foldl x (cons op l)) (seq_foldl x (snoc l op)))) (decreases length l) =
  match length l with
  |0 -> lemma_empty l; 
       append_empty_r (create 1 op); 
       append_empty_l (create 1 op);
       assert (forall x. foldl_prop x (cons op l) /\ foldl_prop x (snoc l op) ==>
                    (seq_foldl x (cons op l) = seq_foldl x (snoc l op)));
  ...
  |_ -> let hd = head l in let tl = tail l in
       ...
       assert (forall x. foldl_prop x (cons op l) /\ foldl_prop x (snoc l op) ==> 
                    eq (seq_foldl (do x hd) (cons op tl)) (seq_foldl (do x hd) (snoc tl op))); // from IH
       
       lem_case4 l op;
       lem_case4_eq l op;
                    // eq (seq_foldl (do x hd) (snoc tl op)) (seq_foldl x (snoc l op)));
       
       lem_eq_is_equiv_c1 l op; 
       lem_eq_is_equiv_c2 l op;
       lem_eq_is_equiv_c3 l op;
       lem_eq_is_equiv_c4 l op

let lem_seq_foldl_split (x:concrete_st) (p:log) (s:log)
  : Lemma (requires foldl_prop x (p ++ s))
          (ensures foldl_prop x p /\ foldl_prop (seq_foldl x p) s /\
                   seq_foldl (seq_foldl x p) s = (seq_foldl x (p ++ s)))
          (decreases length p) =
  lem_is_diff (p ++ s) p s;
  split_prefix x p (p ++ s)

val lem_fold_lastop_help (x:concrete_st) (hd:log_entry) (tl:log)
  : Lemma (requires concrete_do_pre x hd /\ foldl_prop (do x hd) tl)
          (ensures foldl_prop x (cons hd tl))

let rec lem_fold_lastop (x:concrete_st) (p:log) (lastop:log_entry)
  : Lemma (requires foldl_prop x p /\ concrete_do_pre (seq_foldl x p) lastop)
          (ensures (foldl_prop x (snoc p lastop))) (decreases length p) =
  match length p with
  |0 -> ()
  |1 -> ()
  |_ -> lem_seq_foldl x p;
       assert (foldl_prop (do x (head p)) (tail p));
       assert (concrete_do_pre (seq_foldl (do x (head p)) (tail p)) lastop);      
       lem_fold_lastop (do x (head p)) (tail p) lastop;
       assert (foldl_prop (do x (head p)) (snoc (tail p) lastop));
       lem_fold_lastop_help x (head p) (snoc (tail p) lastop);
       assert (foldl_prop x (cons (head p) (snoc (tail p) lastop)));
       mem_cons (head p) (tail p);
       assert (p = cons (head p) (tail p));
       append_assoc (create 1 (head p)) (tail p) (create 1 lastop);
       assert (snoc p lastop = (cons (head p) (snoc (tail p) lastop)));
       ()

val lem_foldl_prop_help (x:concrete_st) (p s:log)
...
let rec lem_foldl_prop (x:concrete_st) (p s:log)
  : Lemma (requires foldl_prop x p /\ foldl_prop (seq_foldl x p) s /\ distinct_ops (p ++ s))
          (ensures foldl_prop x (p ++ s)) (decreases length s) =
  match length s with
  |0 -> lemma_empty s; append_empty_r p
  |_ -> assert (concrete_do_pre (seq_foldl x p) (head s));
       lem_fold_lastop x p (head s);
       assert (foldl_prop x (snoc p (head s)));
       lem_foldl_prop_help x p s;
       assert (foldl_prop (seq_foldl x (snoc p (head s))) (tail s)); 
       assume (distinct_ops (snoc p (head s) ++ tail s)); //todo, can be proved
       lem_foldl_prop x (snoc p (head s)) (tail s);
...
let lem_seq_foldl_op (x:concrete_st) (l:log) (op:log_entry)
  ...
  lem_cons_snoc (seq_foldl x pre) suf op;
  assert (foldl_prop (seq_foldl x pre) (snoc suf op));
  ...
  lem_foldl_prop x pre (snoc suf op);
  assert (foldl_prop x (pre ++ snoc suf op)); 

  append_assoc pre suf (create 1 op); 
  assert (foldl_prop x (snoc (pre ++ suf) op));
