...
let linearizable_gt0_inv2_c2''_ind (lca s1 s2:st) (last2:log_entry)
  : Lemma (requires common_pre2 lca s1 s2 /\
                    length (ops_of s2) > length (ops_of lca) /\
                    Add? (snd last2) /\
                    not (exists_triple last2 (diff (ops_of s1) (ops_of lca))) /\
...
let pre1_pre2_s2 (lca s1 s2:st)
    : Lemma (requires common_pre1 lca s1 s2)
            (ensures common_pre2 lca s1 (inverse_st s2)) = 
  lem_inverse (ops_of lca) (ops_of s2);
  lastop_diff (ops_of lca) (ops_of s2);
  inverse_diff_id1 (ops_of lca) (ops_of s1) (ops_of s2)
...
let linearizable_gt0_inv2_c2''_s2_0_base (lca s1 s2:st) (last2:log_entry)
  : Lemma (requires common_pre2 lca s1 s2 /\
...
let linearizable_gt0_inv2_c2''_s2_0_ind_l1_rem (lca s1 s2:st) (last2:log_entry)
  : Lemma (requires common_pre2 lca s1 s2 /\
                    ops_of s2 = ops_of lca /\ 
                    length (ops_of s1) > length (ops_of lca) /\
                    (let _, last1 = un_snoc (ops_of s1) in
...
let linearizable_gt0_inv2_c2''_s2_0_ind_l1_add (lca s1 s2:st) (last2:log_entry)
  : Lemma (requires common_pre2 lca s1 s2 /\
                    ops_of s2 = ops_of lca /\ 
                    length (ops_of s1) > length (ops_of lca) /\
                    (let _, last1 = un_snoc (ops_of s1) in
...
let linearizable_gt0_inv2_c2''_s2_0_ind_l1_rem_eq (lca s1 s2:st) (last2:log_entry)
  : Lemma (requires common_pre2 lca s1 s2 /\
                    ops_of s2 = ops_of lca /\ 
                    length (ops_of s1) > length (ops_of lca) /\
                    (let _, last1 = un_snoc (ops_of s1) in
  ...
  //this case will not occur as not (exists_triple last1 (diff (ops_of s2) (ops_of lca))))
                      
let linearizable_gt0_inv2_c2''_s2_0_ind (lca s1 s2:st) (last2:log_entry)
  : Lemma (requires common_pre2 lca s1 s2 /\
                    ops_of s2 = ops_of lca /\ 
                    length (ops_of s1) > length (ops_of lca) /\
                    Add? (snd last2) /\
  ...
  let _, last1 = un_snoc (ops_of s1) in
  if Rem? (snd last1) && get_ele last1 <> get_ele last2 then
    linearizable_gt0_inv2_c2''_s2_0_ind_l1_rem lca s1 s2 last2
  else if Add? (snd last1) then
    linearizable_gt0_inv2_c2''_s2_0_ind_l1_add lca s1 s2 last2 
  else linearizable_gt0_inv2_c2''_s2_0_ind_l1_rem_eq lca s1 s2 last2

type common_pre3 (lca s1 s2:st) =
  is_prefix (ops_of lca) (ops_of s1) /\
  is_prefix (ops_of lca) (ops_of s2) /\
  Seq.length (ops_of s1) > Seq.length (ops_of lca) /\
  concrete_merge_pre (v_of lca) (v_of s1) (v_of s2) /\
...
let pre1_pre2_s1 (lca s1 s2:st)
    : Lemma (requires common_pre3 lca s1 s2)
            (ensures common_pre2 lca (inverse_st s1) s2) = 
  lem_inverse (ops_of lca) (ops_of s1);
  lastop_diff (ops_of lca) (ops_of s1);
  inverse_diff_id (ops_of lca) (ops_of s1) (ops_of s2)
...
let rec linearizable_gt0_inv2_c2''_s2_0 (lca s1 s2:st) (last2:log_entry)
  : Lemma (requires common_pre2 lca s1 s2 /\
                    ops_of s2 = ops_of lca /\
                    Add? (snd last2) /\
                    not (exists_triple last2 (diff (ops_of s1) (ops_of lca))))
  ...
  if ops_of s1 = ops_of s2 then 
    linearizable_gt0_inv2_c2''_s2_0_base lca s1 s2 last2
  else 
    (assert (length (ops_of s1) > length (ops_of lca));
     let s1' = inverse_st s1 in
     ...
     linearizable_gt0_inv2_c2''_s2_0 lca s1' s2 last2;
     linearizable_gt0_inv2_c2''_s2_0_ind lca s1 s2 last2)

let rec linearizable_gt0_inv2_c2'' (lca s1 s2:st) (last2:log_entry)
  : Lemma (requires common_pre2 lca s1 s2 /\
                    Add? (snd last2) /\
                    not (exists_triple last2 (diff (ops_of s1) (ops_of lca))))
          (ensures eq (do (concrete_merge (v_of lca) (v_of s1) (v_of s2)) last2)
  ...
  if ops_of s2 = ops_of lca then
    ...
    (assert (length (ops_of s2) > length (ops_of lca));
     ...
     assert (common_pre2 lca s1 s2');
     linearizable_gt0_inv2_c2'' lca s1 s2' last2;
     linearizable_gt0_inv2_c2''_ind lca s1 s2 last2)
