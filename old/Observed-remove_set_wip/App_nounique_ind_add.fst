...
val filter : f:((nat * nat) -> bool)
           ...
           -> Tot (l1:concrete_st {(forall e. L.mem e l1 /\ mem_id_s (fst e) l1 <==> L.mem e l /\ mem_id_s (fst e) l /\ f e) /\
                                  (forall e. L.mem e l /\ f e ==> (find_ele (fst e) l = find_ele (fst e) l1 /\
                                        L.mem e l /\ L.mem e l1))})
let rec filter f l = 
  match l with
  |[] -> []
...
let do (s:concrete_st) (o:log_entry{concrete_do_pre s o}) 
    : (r:concrete_st{(Add? (snd o) ==> (forall ele. mem_ele ele r <==> (mem_ele ele s \/ ele = get_ele o)) /\
                     ...
                     (forall id. mem_id_s id s /\ mem_id_s id r ==> find_ele id s = find_ele id r*) }) =
  match o with
  |(id, Add e) -> (id, e)::s
...
let do_prop (s:concrete_st) (o:log_entry{concrete_do_pre s o}) 
  : Lemma (ensures (Add? (snd o) ==> ((forall ele. mem_ele ele (do s o) <==> (mem_ele ele s \/ ele = get_ele o)) /\
                   ...
                   (forall id. mem_id_s id s /\ mem_id_s id (do s o) ==> find_ele id s = find_ele id (do s o)*)) = ()
...
let rec remove (l:concrete_st) (ele:(nat * nat)) 
  : Tot (res:concrete_st{(forall e. L.mem e res <==> L.mem e l /\ e <> ele) /\ not (L.mem ele res) 
                         ...
                         (forall id. mem_id_s id l /\ id <> fst ele ==> (find_ele id l = find_ele id res /\
                            L.mem (id, (find_ele id l)) l /\  L.mem (id, (find_ele id res)) res /\
                             L.mem (id, (find_ele id l)) res /\  L.mem (id, (find_ele id res)) l)*)}) =
  match l with
  |[] -> []
...
(*let rec lem_find (a:concrete_st)
  : Lemma (requires true)
          (ensures (forall e. L.mem e a ==> snd e = find_ele (fst e) a) /\
                   (forall id. mem_id_s id a ==> L.mem (id, (find_ele id a)) a)) (decreases a) =
  match a with
  |[] -> ()
  |x::xs -> lem_find xs*)
...
(*let lem_find_ele (lca s1:st)
  : Lemma (requires is_prefix (ops_of lca) (ops_of s1) /\
                    (forall id. mem_id id (ops_of lca) ==> not (mem_id id (diff (ops_of s1) (ops_of lca)))))
          (ensures (forall id. mem_id_s id (v_of s1) /\ mem_id_s id (v_of lca) ==> find_ele id (v_of s1) = find_ele id (v_of lca))) =
  ...
  lem_add_init (ops_of lca);
  assert (forall e. L.mem e (v_of lca) ==> mem (fst e, Add (snd e)) (ops_of lca));
  assert (forall id ele. L.mem (id, ele) (v_of lca) ==> mem (id, Add ele) (ops_of lca)); 
  //lem_find (v_of lca);
  assert (forall e. L.mem e (v_of lca) ==> snd e = find_ele (fst e) (v_of lca));
  assert (forall id. mem_id_s id (v_of lca) ==> mem_id id (ops_of lca) /\ L.mem (id, (find_ele id (v_of lca))) (v_of lca));
  lem_id_ele (v_of lca) (ops_of lca);
  assert (forall id. mem_id_s id (v_of lca) ==> mem (id, Add (find_ele id (v_of lca))) (ops_of lca)); 
  assert (forall e. mem e (ops_of lca) ==> mem e (ops_of s1)); 
  assert (forall id. mem_id_s id (v_of lca) ==> mem (id, Add (find_ele id (v_of lca))) (ops_of s1));
  
  lem_add_init (ops_of s1);
  assert (forall e. L.mem e (v_of s1) ==> mem (fst e, Add (snd e)) (ops_of s1));
  assert (forall id ele. L.mem (id, ele) (v_of s1) ==> mem (id, Add ele) (ops_of s1));
  //lem_find (v_of s1);
  assert (forall e. L.mem e (v_of s1) ==> snd e = find_ele (fst e) (v_of s1));
  assert (forall id. mem_id_s id (v_of s1) ==> mem_id id (ops_of s1) /\ L.mem (id, (find_ele id (v_of s1))) (v_of s1));
  lem_id_ele (v_of s1) (ops_of s1);
  assert (forall id. mem_id_s id (v_of s1) ==> mem (id, Add (find_ele id (v_of s1))) (ops_of s1));
                                          
  assert (forall id. mem_id_s id (v_of s1) /\ mem_id_s id (v_of lca) ==>
                mem (id, Add (find_ele id (v_of lca))) (ops_of lca) /\ mem (id, Add (find_ele id (v_of s1))) (ops_of s1)); 
  assert (forall id. mem_id_s id (v_of s1) /\ mem_id_s id (v_of lca) ==> mem (id, Add (find_ele id (v_of s1))) (ops_of s1));
  assert (forall e. mem e (ops_of s1) <==> mem e (ops_of lca) \/ mem e (diff (ops_of s1) (ops_of lca)));
  
  assert (forall id. mem_id_s id (v_of s1) /\ mem_id_s id (v_of lca) /\ (find_ele id (v_of s1)) <> (find_ele id (v_of lca)) ==>
             mem (id, Add (find_ele id (v_of lca))) (ops_of s1) /\
             mem (id, Add (find_ele id (v_of s1))) (ops_of s1)); 
  lem_two_id (ops_of s1); 
  assert (forall id. mem_id_s id (v_of s1) /\ mem_id_s id (v_of lca) /\ (find_ele id (v_of s1)) <> (find_ele id (v_of lca)) ==>
                (count_id id (ops_of s1) > 1));
  ()

let rec lem_diff_s (a l:concrete_st)
  : Lemma (requires (forall id. mem_id_s id a /\ mem_id_s id l ==> find_ele id a = find_ele id l))
          (ensures (forall id. mem_id_s id a /\ not (mem_id_s id l) ==> (find_ele id a = find_ele id (diff_s a l) /\
                       L.mem (id, (find_ele id (diff_s a l))) (diff_s a l) /\  L.mem (id, (find_ele id a)) a /\
                       L.mem (id, (find_ele id a)) (diff_s a l) /\  L.mem (id, (find_ele id a)) a)) /\
                       (forall id. mem_id_s id a /\ not (mem_id_s id l) <==> mem_id_s id (diff_s a l)))  (decreases a) =
   match a with
  |[] -> ()
  |x::xs -> if L.mem x l then lem_diff_s xs (remove l x) else lem_diff_s xs l

let lem_st_seq_help1 (lca s1:st)
  : Lemma (requires is_prefix (ops_of lca) (ops_of s1) /\
  ...
  assert (forall id. mem_id_s id (diff_s (v_of s1) (v_of lca)) <==> (mem_id_s id (v_of s1) /\ not (mem_id_s id (v_of lca)))); 
...
(*let lem_st_seq_help (lca s1 s2:st)
  : Lemma (requires is_prefix (ops_of lca) (ops_of s1) /\
  ...
  //lem_st_seq_help1 lca s1;
  //lem_st_seq_help1 lca s2;
  lem_st_seq_help' (diff_s (v_of s1) (v_of lca)) (diff_s (v_of s2) (v_of lca)) (diff (ops_of s1) (ops_of lca)) (diff (ops_of s2) (ops_of lca))*)
...
(*let rec lem_foldl_prop1 (x:concrete_st) (p s:log)
  : Lemma (requires foldl_prop x p /\ foldl_prop (seq_foldl x p) s /\ distinct_ops (p ++ s)
                    (*forall id. mem_id id p ==> not (mem_id id s*))
          (ensures foldl_prop x (p ++ s)) (decreases length p) =
  match Seq.length p with
    |0 -> admit ()
    |1 -> admit ()
    |_ -> let pre, lastop = un_snoc p in
         assume (foldl_prop x pre /\
                 foldl_prop (seq_foldl x pre) (cons lastop s) /\
                 distinct_ops (pre ++ cons lastop s));
         lem_foldl_prop1 x pre (cons lastop s);
         assume (p ++ s = pre ++ cons lastop s); ()*)
...
let merge_inv_prop (lca s1 s2:st)
  ...
  if exists_triple last1 (diff (ops_of s2) (ops_of lca)) 
    ...
    then merge_inv_prop_inv1' lca s1 s2
          ...
          last (resolve_conflict last1 last2) = last1
    then merge_inv_prop_inv1 lca s1 s2
  else merge_inv_prop_inv2 lca s1 s2
...
let linearizable_gt0_inv2' (lca s1 s2:st)
  ...
  let inv2 = inverse_st_op s2 op in
  assume (forall e. L.mem e (v_of inv2) <==> (L.mem e (v_of s2) /\ e <> (fst op, get_ele op))); //todo
  lem_suf_equal2 (ops_of lca) (ops_of s2) op;
  assert (not (mem_id (fst op) (ops_of lca)));
  lem_foldl init_st (ops_of lca);
...
let linearizable_gt0_inv1' (lca s1 s2:st)
  ...
  let (_, op, _) = find_triple last2 (diff (ops_of s1) (ops_of lca)) in
  let inv1 = inverse_st_op s1 op in
  assume (forall e. L.mem e (v_of inv1) <==> (L.mem e (v_of s1) /\ e <> (fst op, get_ele op))); //todo
...
let linearizable_gt0_inv1 (lca s1 s2:st)
  ...
  assume (forall e. L.mem e (v_of inv1) <==> (L.mem e (v_of s1) /\ e <> (fst last1, get_ele last1))); //todo
...
let linearizable_gt0_inv2_c3'_s1_0_ind (lca s1 s2:st) (last2:log_entry)
  : Lemma (requires common_pre1 lca s1 s2 /\ 
                   ops_of s1 = ops_of lca /\
                   Rem? (snd last2) /\
                   not (mem_id (fst last2) (ops_of lca)) /\
...
let linearizable_gt0_inv2_c3'_s1_0_base (lca s1 s2:st) (last2:log_entry)
  : Lemma (requires common_pre2 lca s1 s2 /\ 
                   ops_of s1 = ops_of lca /\
                   Rem? (snd last2) /\
                   concrete_do_pre (v_of s2) last2 /\
...
let rec linearizable_gt0_inv2_c3'_s1_0 (lca s1 s2:st) (last2:log_entry)
 : Lemma (requires common_pre2 lca s1 s2 /\ 
                   ops_of s1 = ops_of lca /\
                   Rem? (snd last2) /\
                   not (mem_id (fst last2) (ops_of lca)) /\
   ...
   if ops_of s2 = ops_of lca
     then linearizable_gt0_inv2_c3'_s1_0_base lca s1 s2 last2
   else 
     (assert (length (ops_of s2) > length (ops_of lca));
      let s2' = inverse_st s2 in
      common_pre1_pre2 lca s1 s2;
      lem_common_pre1_s2' lca s1 s2 last2;
      linearizable_gt0_inv2_c3'_s1_0 lca s1 s2' last2;
      linearizable_gt0_inv2_c3'_s1_0_ind lca s1 s2 last2)    

let linearizable_gt0_inv2_c3'_pre (lca s1 s2:st)
  : Lemma (requires common_pre1 lca s1 s2 /\ 
                   (let _, last2 = un_snoc (ops_of s2) in
                    Rem? (snd last2) /\
                    not (exists_triple last2 (diff (ops_of s1) (ops_of lca))) /\
...
let rec linearizable_gt0_inv2_c3' (lca s1 s2:st)
 : Lemma (requires common_pre1 lca s1 s2 /\ 
                   (let _, last2 = un_snoc (ops_of s2) in
                    Rem? (snd last2) /\
                    not (exists_triple last2 (diff (ops_of s1) (ops_of lca))) /\
   ...
   if ops_of s1 = ops_of lca then
     (let s2' = inverse_st s2 in
      linearizable_gt0_inv2_c3'_pre lca s1 s2;
      linearizable_gt0_inv2_c3'_s1_0 lca s1 s2' last2) 
   else 
     (assert ((length (ops_of s1) > length (ops_of lca)));
      ...
      if Rem? (snd last1) && get_ele last1 = get_ele last2 then
        ...
        (ind_case_last1_neq_pre1 lca s1 s2;
         ...
         linearizable_gt0_inv2_c3' lca s1' s2;
...
let lem_mem_of (l a a':concrete_st) (ele:(nat * nat))
    : Lemma (requires (a = ele::a') /\ 
                     // (a = (diff_s a l) @ l) /\
                     // (a' = (diff_s a' l) @ l))
                     (forall e. L.mem e a <==> L.mem e a' \/ e = ele) /\
                      (forall e. L.mem e a' <==> L.mem e a /\ e <> ele) /\
                      (forall id. mem_id_s id a <==> (mem_id_s id a' \/ id = fst ele)) /\
                      (forall id. mem_id_s id a' <==> (mem_id_s id a /\ id <> fst ele)) /\
                      (forall e. L.mem e (diff_s a' l) ==> L.mem e a' /\ not (L.mem e l)) /\
                      (forall e. L.mem e (diff_s a' l) ==> L.mem e a /\ not (L.mem e l)) /\
                      (forall e. L.mem e (diff_s a l) ==> L.mem e a /\ not (L.mem e l)) /\
                      not (L.mem ele a') /\ not (L.mem ele (diff_s a' l)) /\
                      (forall e. L.mem e (diff_s a' l) ==> L.mem e (diff_s a l)) /\
                      not (mem_id_s (fst ele) a'))
                      //not (mem_id_s (fst ele) l))
            (ensures not (L.mem ele l)) = ()
              //(L.mem ele (diff_s a l))) (decreases l) = ()*)


#push-options "--z3rlimit 1000 --fuel 1 --ifuel 1"
let linearizable_gt0_inv2_c2'_s1_0_ind (lca s1 s2:st) (last2:log_entry)
  : Lemma (requires common_pre1 lca s1 s2 /\ 
                   ops_of s1 = ops_of lca /\
                   Add? (snd last2) /\
                   not (mem_id (fst last2) (ops_of lca)) /\
...
let linearizable_gt0_inv2_c2'_s1_0_base (lca s1 s2:st) (last2:log_entry)
  : Lemma (requires common_pre2 lca s1 s2 /\ 
                   ops_of s1 = ops_of lca /\
                   Add? (snd last2) /\
                   concrete_do_pre (v_of s2) last2 /\
...
let rec linearizable_gt0_inv2_c2'_s1_0 (lca s1 s2:st) (last2:log_entry)
 : Lemma (requires common_pre2 lca s1 s2 /\ 
                   ops_of s1 = ops_of lca /\
                   Add? (snd last2) /\
                   not (mem_id (fst last2) (ops_of lca)) /\
         ...
         (decreases %[length (ops_of s2)]) =
   if ops_of s2 = ops_of lca
     then linearizable_gt0_inv2_c2'_s1_0_base lca s1 s2 last2
   else 
     (assert (length (ops_of s2) > length (ops_of lca));
      let s2' = inverse_st s2 in
      common_pre1_pre2 lca s1 s2;
      lem_common_pre1_s2' lca s1 s2 last2;
      linearizable_gt0_inv2_c2'_s1_0 lca s1 s2' last2;
      linearizable_gt0_inv2_c2'_s1_0_ind lca s1 s2 last2)              

let linearizable_gt0_inv2_c2'_pre (lca s1 s2:st)
  : Lemma (requires common_pre1 lca s1 s2 /\ 
                   (let _, last2 = un_snoc (ops_of s2) in
                    Add? (snd last2) /\
                    not (exists_triple last2 (diff (ops_of s1) (ops_of lca))) /\
...
let ind_case_c2_pre4 (lca s1 s2:st)
 : Lemma (requires common_pre1 lca s1 s2 /\ 
                   length (ops_of s1) > length (ops_of lca) /\
                   (let _, last2 = un_snoc (ops_of s2) in
                    let _, last1 = un_snoc (ops_of s1) in
...
let lem_last_add_diff (lca s1:st)
  ...
  assume (forall e. L.mem e (v_of s1') <==> (L.mem e (v_of s1) /\ e <> (fst last1, get_ele last1))); //todo 
...
let ind_case_c2_last1_add (lca s1 s2:st)
  : Lemma (requires (Seq.length (ops_of s1) > Seq.length (ops_of lca) /\
                    (let s1' = inverse_st s1 in
                    common_pre1 lca s1 s2 /\
                    (let s2' = inverse_st s2 in
  ...
  lem_last_add_diff lca s2;

  do_prop (concrete_merge (v_of lca) (v_of s1') (v_of s2')) last2;
  do_prop (concrete_merge (v_of lca) (v_of s1) (v_of s2')) last2;
  assert (forall e. L.mem e (do (concrete_merge (v_of lca) (v_of s1') (v_of s2')) last2) \/ e = (fst last1, get_ele last1) <==>
               L.mem e (do (concrete_merge (v_of lca) (v_of s1) (v_of s2')) last2));
  assert (forall e. L.mem e (concrete_merge (v_of lca) (v_of s1') (v_of s2)) \/ e = (fst last1, get_ele last1) <==>
               L.mem e (concrete_merge (v_of lca) (v_of s1) (v_of s2)));
...
let ind_case_c2_last1_rem (lca s1 s2:st)
  : Lemma (requires (Seq.length (ops_of s1) > Seq.length (ops_of lca) /\
                    (let s1' = inverse_st s1 in
                    common_pre1 lca s1 s2 /\
                    (let s2' = inverse_st s2 in
  ...
  do_prop (concrete_merge (v_of lca) (v_of s1) (v_of s2')) last2;
  assert (forall e. (L.mem e (concrete_merge (v_of lca) (v_of s1) (v_of s2')) \/ e = (fst last2, get_ele last2)) <==>
                L.mem e (do (concrete_merge (v_of lca) (v_of s1) (v_of s2')) last2));
  ...
  : Lemma (requires (Seq.length (ops_of s1) > Seq.length (ops_of lca) /\
          ...
          (ensures (let _, last2 = un_snoc (ops_of s2) in
                    ...
                    eq (do (concrete_merge (v_of lca) (v_of s1) (v_of s2')) last2)
                       (concrete_merge (v_of lca) (v_of s1) (v_of s2)))) =
  let _, last1 = un_snoc (ops_of s1) in
  if Add? (snd last1) then ind_case_c2_last1_add lca s1 s2
    else ind_case_c2_last1_rem lca s1 s2
    
let rec linearizable_gt0_inv2_c2' (lca s1 s2:st)
 : Lemma (requires common_pre1 lca s1 s2 /\ 
                   (let _, last2 = un_snoc (ops_of s2) in
                    Add? (snd last2) /\
                    not (exists_triple last2 (diff (ops_of s1) (ops_of lca))) /\
   ...
   if ops_of s1 = ops_of lca then
     (let s2' = inverse_st s2 in
      let _, last2 = un_snoc (ops_of s2) in
      linearizable_gt0_inv2_c2'_pre lca s1 s2;
      linearizable_gt0_inv2_c2'_s1_0 lca s1 s2' last2)       
   else 
     (assert ((length (ops_of s1) > length (ops_of lca)));
      let _, last1 = un_snoc (ops_of s1) in
      ...
      ind_case_last1_neq_pre2 lca s1 s2;
      ind_case_last1_neq_pre3 lca s1 s2;
      ind_case_c2_pre4 lca s1 s2;
      linearizable_gt0_inv2_c2' lca s1' s2;
      ind_case_c2 lca s1 s2)



//****************************************************//
let common_pre1'' (lca s1 s2:st) =
  is_prefix (ops_of lca) (ops_of s1) /\
  is_prefix (ops_of lca) (ops_of s2) /\
  Seq.length (ops_of s2) > Seq.length (ops_of lca) /\
  (forall id id1. mem_id id (ops_of lca) /\ mem_id id1 (diff (ops_of s1) (ops_of lca)) ==> lt id id1) /\
...
let linearizable_gt0_inv2_c2''_s1_0_s2_0_base (lca s1 s2:st) (last1:log_entry) (last2:log_entry)
  : Lemma (requires common_pre2'' lca s1 s2 /\ 
...
let linearizable_gt0_inv2_c2''_s1_0_s2_0_s2'_ind (lca s1 s2:st) (last1:log_entry) (last2:log_entry)
  : Lemma (requires common_pre1'' lca s1 s2 /\ 
                   ops_of s1 = ops_of lca /\ 
                   Add? (snd last2) /\ Rem? (snd last1) /\ get_ele last1 <> get_ele last2 /\
                   not (mem_id (fst last2) (ops_of lca)) /\ not (mem_id (fst last1) (ops_of lca)) /\
                   not (mem_id (fst last2) (ops_of s1)) /\ not (mem_id (fst last1) (ops_of s1)) /\
                   not (mem_id (fst last2) (ops_of s2)) /\ not (mem_id (fst last1) (ops_of s2)) /\
                   ...
                   //not (exists_triple last1 (diff (ops_of s1) (ops_of lca))) /\
...
#push-options "--z3rlimit 1500 --fuel 1 --ifuel 1"

let test (lca s1 s2:st) (last2:log_entry)
  : Lemma (requires common_pre1 lca s1 s2 /\ 
                   //ops_of s1 = ops_of lca /\
                   Add? (snd last2) /\
                   not (mem_id (fst last2) (ops_of lca)) /\
...
let test2 (lca s1 s2:st) (last2:log_entry)
  : Lemma (requires common_pre1 lca s1 s2 /\ 
                   Add? (snd last2) /\
                   not (mem_id (fst last2) (ops_of lca)) /\
                   not (mem_id (fst last2) (ops_of s1)) /\
  ...
  : Lemma (requires (Seq.length (ops_of s1) > Seq.length (ops_of lca) /\
          ...
          (ensures (let _, last2 = un_snoc (ops_of s2) in
                    ...
                    eq (do (concrete_merge (v_of lca) (v_of s1) (v_of s2')) last2)
                       (concrete_merge (v_of lca) (v_of s1) (v_of s2)))) =
  (*let _, last2 = un_snoc (ops_of s2) in
  assume (L.mem (fst last2, get_ele last2) (diff_s (v_of s2) (v_of lca)));*)
  ()
...
let linearizable_gt0_inv2_c2''_s1_0_s2_0_s2'_ind (lca s1 s2:st) (last1:log_entry) (last2:log_entry)
  ...
  assume (forall e. L.mem e (concrete_merge (v_of lca) (v_of s1) (v_of s2')) <==> L.mem e (v_of s2'));
  assume (forall e. L.mem e (do (concrete_merge (v_of lca) (v_of s1) (v_of s2')) last1) <==> 
               L.mem e (v_of s2') /\ snd e <> (get_ele last1)); 
  assume (forall e. L.mem e (do (do (concrete_merge (v_of lca) (v_of s1) (v_of s2')) last1) last2) <==>
               (L.mem e (v_of s2') /\ snd e <> (get_ele last1)) \/ e = (fst last2, get_ele last2));
  ...
  assume (L.mem (fst last2, get_ele last2) (concrete_merge (v_of lca) (do (v_of s1) last1) (do (v_of s2') last2)));
  
  assume (forall e. L.mem e (v_of lca) /\ L.mem e (do (v_of s1) last1) /\ L.mem e (do (v_of s2') last2) ==> snd e <> get_ele last1);

  assume (forall e. L.mem e (v_of lca) /\ L.mem e (do (v_of s1) last1) /\ L.mem e (do (v_of s2) last2) ==> snd e <> get_ele last1); //todo *)
  () 

#push-options "--z3rlimit 1000 --fuel 1 --ifuel 1"
let lem_common_pre1_s2''_s1_0 (lca s1 s2:st) (last1:log_entry) (last2:log_entry)
  : Lemma (requires common_pre1'' lca s1 s2 /\
                    not (mem_id (fst last2) (ops_of s2)) /\ not (mem_id (fst last1) (ops_of s2)) /\
                    not (mem_id (fst last2) (ops_of lca)) /\ not (mem_id (fst last1) (ops_of lca)) /\
                    not (mem_id (fst last2) (ops_of s1)) /\ not (mem_id (fst last1) (ops_of s1)) /\
  ...
  lem_not_id_pre (ops_of s2) (fst last2);
  assert (not (mem_id (fst last2) (ops_of s2')));
  assert (not (mem_id (fst last1) (ops_of s2')));
  ()

let common_pre1_pre2''_s1_0 (lca s1 s2:st)
  : Lemma (requires common_pre2'' lca s1 s2 /\
                    length (ops_of s2) > length (ops_of lca))
          (ensures common_pre1'' lca s1 s2) = ()

#push-options "--z3rlimit 1500 --fuel 1 --ifuel 1"
let trial2 (lca s1 s2:st) (last1:log_entry) (last2:log_entry)
 : Lemma (requires common_pre2'' lca s1 s2 /\ 
                   ops_of s1 = ops_of lca /\ 
                   Add? (snd last2) /\ Rem? (snd last1) /\ get_ele last1 <> get_ele last2 /\
                   not (mem_id (fst last2) (ops_of lca)) /\ not (mem_id (fst last1) (ops_of lca)) /\
          ...
          (ensures eq (do (do (concrete_merge (v_of lca) (v_of s1) (v_of s2)) last1) last2)
                       (concrete_merge (v_of lca) (do (v_of s1) last1) (do (v_of s2) last2))) = 
 assume (L.mem (fst last2, get_ele last2) (diff_s (do (v_of s2) last2) (v_of lca)));  //todo
 ()


let nlinearizable_gt0_inv2_c2''_s1_0_s2_0_base (lca s1 s2:st) (last1:log_entry) (last2:log_entry)
  : Lemma (requires common_pre2'' lca s1 s2 /\ 
                    ops_of s1 = ops_of lca /\ ops_of s2 = ops_of lca /\
                    Add? (snd last2) /\ Rem? (snd last1) /\ get_ele last1 = get_ele last2 /\
                    not (exists_triple last2 (diff (ops_of s1) (ops_of lca))))
          (ensures eq (do (do (concrete_merge (v_of lca) (v_of s1) (v_of s2)) last1) last2)
                       (concrete_merge (v_of lca) (do (v_of s1) last1) (do (v_of s2) last2))) = 
  assume (L.mem (fst last2, get_ele last2) (diff_s (do (v_of s2) last2) (v_of lca))); //todo - enough
  ()

#push-options "--z3rlimit 1000 --fuel 1 --ifuel 1"
let nlinearizable_gt0_inv2_c2''_s1_0_s2_0_s2'_ind (lca s1 s2:st) (last1:log_entry) (last2:log_entry)
  : Lemma (requires common_pre1'' lca s1 s2 /\ 
                   ops_of s1 = ops_of lca /\ 
                   Add? (snd last2) /\ Rem? (snd last1) /\ get_ele last1 = get_ele last2 /\
                   not (mem_id (fst last2) (ops_of lca)) /\ not (mem_id (fst last1) (ops_of lca)) /\
  ...
  assume (forall e. L.mem e (concrete_merge (v_of lca) (v_of s1) (v_of s2')) <==> L.mem e (v_of s2'));
  assume (forall e. L.mem e (do (concrete_merge (v_of lca) (v_of s1) (v_of s2')) last1) <==> 
               L.mem e (v_of s2') /\ snd e <> (get_ele last2));
  assume (forall e. L.mem e (do (do (concrete_merge (v_of lca) (v_of s1) (v_of s2')) last1) last2) <==>
               (L.mem e (v_of s2') /\ snd e <> (get_ele last2)) \/ e = (fst last2, get_ele last2));
  ...
  assume (L.mem (fst last2, get_ele last2) (concrete_merge (v_of lca) (do (v_of s1) last1) (do (v_of s2') last2)));
  assume (not (mem_ele (get_ele last2) (do (v_of s1) last1)));
  assume (not (mem_ele (get_ele last2) (diff_s (do (v_of s1) last1) (v_of lca)))); 
  assume (forall e. L.mem e (v_of lca) /\ L.mem e (do (v_of s1) last1) /\ L.mem e (do (v_of s2') last2) ==> snd e <> get_ele last2);
...
#push-options "--z3rlimit 1000 --fuel 1 --ifuel 1"
let nlem_common_pre1_s2''_s1_0 (lca s1 s2:st) (last1:log_entry) (last2:log_entry)
  : Lemma (requires common_pre1'' lca s1 s2 /\
                    not (mem_id (fst last2) (ops_of s2)) /\ not (mem_id (fst last1) (ops_of s2)) /\
                    not (mem_id (fst last2) (ops_of lca)) /\ not (mem_id (fst last1) (ops_of lca)) /\
                    not (mem_id (fst last2) (ops_of s1)) /\ not (mem_id (fst last1) (ops_of s1)) /\
...
let ncommon_pre1_pre2''_s1_0 (lca s1 s2:st)
  : Lemma (requires common_pre2'' lca s1 s2 /\
                    length (ops_of s2) > length (ops_of lca))
          (ensures common_pre1'' lca s1 s2) = ()

(*let lem_ele_not_merge (l s1 s1' s2:concrete_st) (ele:nat)
    : Lemma (requires (forall e. L.mem e (diff_s s1 l) <==> (L.mem e (diff_s s1' l) /\ snd e <> ele)) /\
                      (forall e. ((L.mem e l /\ L.mem e s1' /\ L.mem e s2) /\ snd e <> ele) <==>
                            (L.mem e l /\ L.mem e s1 /\ L.mem e s2)) /\
                      (forall e. not (mem_ele ele (diff_s s2 l))))
            (ensures (forall e. (L.mem e (concrete_merge l s1' s2) /\ snd e <> ele) <==>
                           L.mem e (concrete_merge l s1 s2))) = ()
                           
let help (lca s1 s2 s1':concrete_st) (op:log_entry)
  : Lemma (requires s1 = do s1' op /\ Rem? (snd op) /\
                    lca = s1' /\ (forall e. L.mem e (concrete_merge lca s1' s2) <==> L.mem e s2) /\
                    (forall e. L.mem e (diff_s s1 lca) <==> (L.mem e (diff_s s1' lca) /\ snd e <> get_ele op)) /\
                    (forall e. ((L.mem e lca /\ L.mem e s1' /\ L.mem e s2) /\ snd e <> get_ele op) <==>
                          (L.mem e lca /\ L.mem e s1 /\ L.mem e s2)) /\
                    (forall e. not (mem_ele (get_ele op) (diff_s s2 lca))))
          (ensures (forall e. (L.mem e (concrete_merge lca s1' s2) /\ snd e <> get_ele op) <==>
                         L.mem e (concrete_merge lca s1 s2))) = ()*)
...
let trial2 (lca s1 s2:st) (last1:log_entry) (last2:log_entry)
 ...
 assume (forall e. L.mem e (do (do (concrete_merge (v_of lca) (v_of s1) (v_of s2)) last1) last2) <==>
              (L.mem e (v_of s2) /\ snd e <> (get_ele last2)) \/ e = (fst last2, get_ele last2));
 assume (forall e. (L.mem e (v_of s2) /\ snd e <> (get_ele last2)) \/ e = (fst last2, get_ele last2) <==>
              (L.mem e (v_of s2) \/ e = (fst last2, get_ele last2)) /\
              (snd e <> get_ele last2 \/ e = (fst last2, get_ele last2)));
 assume (L.mem (fst last2, get_ele last2) (diff_s (do (v_of s2) last2) (v_of lca)));  //todo

 assume (forall e. L.mem e (do (concrete_merge (v_of lca) (v_of s1) (v_of s2)) last1) <==>
              L.mem e (v_of s2) /\ snd e <> get_ele last2);

 assume (forall e. (L.mem e (v_of lca) /\ L.mem e (do (v_of s1) last1) /\ L.mem e (v_of s2)) <==>
              (L.mem e (v_of lca) /\ L.mem e (v_of s1) /\ L.mem e (v_of s2)) /\ snd e <> get_ele last1);

 assume (forall e. L.mem e (do (v_of s1) last1) <==> L.mem e (v_of s1) /\ snd e <> get_ele last1);


 assert (forall e. L.mem e (concrete_merge (v_of lca) (v_of s1) (v_of s2)) /\ snd e <> get_ele last1 <==>
              L.mem e (concrete_merge (v_of lca) (do (v_of s1) last1) (v_of s2))); admit(); //todo
              
 assume (forall e. L.mem e (do (concrete_merge (v_of lca) (v_of s1) (v_of s2)) last1) <==>
              L.mem e (concrete_merge (v_of lca) (do (v_of s1) last1) (v_of s2)));
 
 assume (forall e. L.mem e (do (do (concrete_merge (v_of lca) (v_of s1) (v_of s2)) last1) last2) <==>
              L.mem e (do (concrete_merge (v_of lca) (do (v_of s1) last1) (v_of s2)) last2));
 
 assume (forall e. L.mem e (do (concrete_merge (v_of lca) (do (v_of s1) last1) (v_of s2)) last2) <==>
              L.mem e (concrete_merge (v_of lca) (do (v_of s1) last1) (do (v_of s2) last2))); 
 ()

let rec linearizable_gt0_inv2_c2''_s1_0_s2_0_s2' (lca s1 s2:st) (last1:log_entry) (last2:log_entry)
 : Lemma (requires common_pre2'' lca s1 s2 /\ 
                   ops_of s1 = ops_of lca /\ 
                   Add? (snd last2) /\ Rem? (snd last1) /\ get_ele last1 = get_ele last2 /\
                   not (mem_id (fst last2) (ops_of lca)) /\ not (mem_id (fst last1) (ops_of lca)) /\
         ...
         (decreases %[length (ops_of s2)]) =
   if ops_of s2 = ops_of lca
     then admit() //linearizable_gt0_inv2_c2''_s1_0_s2_0_base lca s1 s2 last1 last2
   else 
     (assume (length (ops_of s2) > length (ops_of lca));
      let s2' = inverse_st s2 in
      common_pre1_pre2''_s1_0 lca s1 s2;
      lem_common_pre1_s2''_s1_0 lca s1 s2 last1 last2;
      linearizable_gt0_inv2_c2''_s1_0_s2_0_s2' lca s1 s2' last1 last2;
      assert (eq (do (do (concrete_merge (v_of lca) (v_of s1) (v_of s2)) last1) last2)
                 (concrete_merge (v_of lca) (do (v_of s1) last1) (do (v_of s2) last2))); admit())
      //linearizable_gt0_inv2_c2''_s1_0_s2_0_s2'_ind lca s1 s2 last1 last2)     

let linearizable_gt0_inv2_c2''_s1_0_s2_0_s1'_ind (lca s1 s2:st) (last1:log_entry) (last2:log_entry)
  : Lemma (requires common_pre3'' lca s1 s2 /\ 
                   Add? (snd last2) /\ Rem? (snd last1) /\ get_ele last1 = get_ele last2 /\
                   not (mem_id (fst last2) (ops_of lca)) /\ not (mem_id (fst last1) (ops_of lca)) /\
                   not (mem_id (fst last2) (ops_of s1)) /\ not (mem_id (fst last1) (ops_of s1)) /\
                   not (mem_id (fst last2) (ops_of s2)) /\ not (mem_id (fst last1) (ops_of s2)) /\
  ...
  let s1' = inverse_st s1 in
  assert (L.mem (fst last2, get_ele last2) (concrete_merge (v_of lca) (do (v_of s1') last1) (do (v_of s2) last2)));
  assert (not (mem_ele (get_ele last2) (do (v_of s1') last1))); 
  assert (not (mem_ele (get_ele last2) (diff_s (do (v_of s1') last1) (v_of lca)))); 
  assert (forall e. L.mem e (v_of lca) /\ L.mem e (do (v_of s1') last1) /\ L.mem e (do (v_of s2) last2) ==> snd e <> get_ele last2);
  assert (L.mem (fst last2, get_ele last2) (diff_s (do (v_of s2) last2) (v_of lca)));
  ()

let common_pre1_pre2''_s1_n0 (lca s1 s2:st)
  : Lemma (requires common_pre2'' lca s1 s2 /\
                    length (ops_of s1) > length (ops_of lca))
          (ensures common_pre3'' lca s1 s2) = ()
          
let lem_common_pre1_s2''_s1_n0 (lca s1 s2:st) (last1:log_entry) (last2:log_entry)
  : Lemma (requires common_pre3'' lca s1 s2 /\
                    not (mem_id (fst last2) (ops_of s2)) /\ not (mem_id (fst last1) (ops_of s2)) /\
                    not (mem_id (fst last2) (ops_of lca)) /\ not (mem_id (fst last1) (ops_of lca)) /\
                    not (mem_id (fst last2) (ops_of s1)) /\ not (mem_id (fst last1) (ops_of s1)) /\
  ...
  assert (forall id id1. mem_id id (ops_of lca) /\ mem_id id1 (diff (ops_of s1') (ops_of lca)) ==> lt id id1);
  assert (forall id id1. mem_id id (ops_of lca) /\ mem_id id1 (diff (ops_of s2) (ops_of lca)) ==> lt id id1);
  lem_not_id_pre (ops_of s1) (fst last2);
  assert (not (mem_id (fst last2) (ops_of s1')));
  assert (not (mem_id (fst last1) (ops_of s1')));
  assume (is_prefix (ops_of lca) (snoc (ops_of s1') last1) ); //todo
  assert (not (exists_triple last2 (diff (snoc (ops_of s1) last1) (ops_of lca)))); 
  ()
  
let rec linearizable_gt0_inv2_c2''_s1_0_s2_0_s1' (lca s1 s2:st) (last1:log_entry) (last2:log_entry)
 : Lemma (requires common_pre2'' lca s1 s2 /\ 
                   Add? (snd last2) /\ Rem? (snd last1) /\ get_ele last1 = get_ele last2 /\
                   not (mem_id (fst last2) (ops_of lca)) /\ not (mem_id (fst last1) (ops_of lca)) /\
                   not (mem_id (fst last2) (ops_of s1)) /\ not (mem_id (fst last1) (ops_of s1)) /\
 ...
 if ops_of s1 = ops_of lca && ops_of s2 = ops_of lca then
   linearizable_gt0_inv2_c2''_s1_0_s2_0_base lca s1 s2 last1 last2
 else if ops_of s1 = ops_of lca then
   linearizable_gt0_inv2_c2''_s1_0_s2_0_s2' lca s1 s2 last1 last2
 else 
     (assert (length (ops_of s1) > length (ops_of lca));
      let s1' = inverse_st s1 in
      common_pre1_pre2''_s1_n0 lca s1 s2;
      lem_common_pre1_s2''_s1_n0 lca s1 s2 last1 last2;
      linearizable_gt0_inv2_c2''_s1_0_s2_0_s1' lca s1' s2 last1 last2;
      linearizable_gt0_inv2_c2''_s1_0_s2_0_s1'_ind lca s1 s2 last1 last2)

let linearizable_gt0_inv1_inv2_c2 (lca s1 s2:st)
 : Lemma (requires common_pre lca s1 s2 /\ 
                   (let _, last1 = un_snoc (ops_of s1) in
                   let _, last2 = un_snoc (ops_of s2) in
                   Add? (snd last2) /\ Rem? (snd last1) /\ get_ele last1 = get_ele last2 /\
 ...
 assert (common_pre2'' lca s1' s2'); 
 lem_id_s2' (ops_of lca) (ops_of s1) (ops_of s2);
 lem_id_s1' (ops_of lca) (ops_of s1) (ops_of s2);
 assume (is_prefix (ops_of lca) (snoc (ops_of s1') last1)); //todo
 linearizable_gt0_inv2_c2''_s1_0_s2_0_s1' lca s1' s2' last1 last2
...
let rec linearizable_gt0_inv2_c2'' (lca s1 s2:st)
 : Lemma (requires common_pre lca s1 s2 /\ 
                   ...
                   (let s2' = inverse_st s2 in
                    ...
                    is_prefix (ops_of lca) (ops_of s1') /\
                    concrete_merge_pre (v_of lca) (v_of s1') (v_of s2') /\
                    concrete_do_pre (concrete_merge (v_of lca) (v_of s1') (v_of s2')) last1 /\
                    concrete_do_pre (do (concrete_merge (v_of lca) (v_of s1') (v_of s2')) last1) last2)))
          (ensures (let _, last2 = un_snoc (ops_of s2) in
                    let _, last1 = un_snoc (ops_of s1) in
                    let s2' = inverse_st s2 in
                    let s1' = inverse_st s1 in
                    eq (do (do (concrete_merge (v_of lca) (v_of s1') (v_of s2')) last1) last2)
                       (concrete_merge (v_of lca) (v_of s1) (v_of s2))))
         (decreases %[length (ops_of s1); length (ops_of s2)]) = admit();
   if ops_of s1 = ops_of lca then
     (let s2' = inverse_st s2 in
      let _, last2 = un_snoc (ops_of s2) in
      linearizable_gt0_inv2_c2'_pre lca s1 s2;
...
let linearizable_gt0 (lca s1 s2:st)
  ...
  if exists_triple last1 (diff (ops_of s2) (ops_of lca)) 
     then admit() //linearizable_gt0_inv2' lca s1 s2
  else if exists_triple last2 (diff (ops_of s1) (ops_of lca)) 
     then (admit() (*assume (not (exists_triple last1 (diff (ops_of s2) (ops_of lca)))); linearizable_gt0_inv1' lca s1 s2*))
  else if last (resolve_conflict last1 last2) = last1
     then (admit() (*assume (not (exists_triple last1 (diff (ops_of s2) (ops_of lca))) /\
                   not (exists_triple last2 (diff (ops_of s1) (ops_of lca)))); linearizable_gt0_inv1 lca s1 s2*))
  else (assume (not (exists_triple last1 (diff (ops_of s2) (ops_of lca))) /\
                not (exists_triple last2 (diff (ops_of s1) (ops_of lca))) /\
                last (resolve_conflict last1 last2) = last2);
    match snd last1, snd last2 with
    |Rem ele1, Add ele -> assert (ele <> ele1); admit() // linearizable_gt0_inv2_c2 lca s1 s2
    |_, Rem _ -> admit()
    |_ -> admit()) //linearizable_gt0_inv2_c3 lca s1 s2)
...
(*let trial (lca s1' s2' s1 s2 s3' s3:st) (last1 last2 op:log_entry)
  : Lemma (requires common_pre2 lca s1' s2' /\
                    common_pre lca s1 s2 /\
                    common_pre2 lca s3' s2' /\
                    common_pre2 lca s3 s2 /\
                    Rem? (snd last1) /\ Add? (snd last2) /\ get_ele last1 = get_ele last2 /\
                    not (mem_id (fst last2) (ops_of lca)) /\ not (mem_id (fst last1) (ops_of lca)) /\
                    not (mem_id (fst last2) (ops_of s1)) /\ ops_of s1 = snoc (ops_of s1') last1 /\
                    not (mem_id (fst last1) (ops_of s2)) /\ ops_of s2 = snoc (ops_of s2') last2 /\
                    not (exists_triple last2 (diff (ops_of s1) (ops_of lca))) /\
                    not (mem_id (fst op) (ops_of s1')) /\
                    not (mem_id (fst last1) (ops_of s3')) /\
                    not (mem_id (fst last1) (ops_of s1')) /\ 
                    v_of s1 = do (v_of s1') last1 /\ v_of s2 = do (v_of s2') last2 /\
                    ops_of s3' = snoc (ops_of s1') op /\
                    ops_of s3 = snoc (ops_of s3') last1 /\
                    v_of s3' = do (v_of s1') op /\ v_of s3 = do (v_of s2') last1 /\
                    eq (do (do (concrete_merge (v_of lca) (v_of s1') (v_of s2')) last1) last2) 
                       (concrete_merge (v_of lca) (v_of s1) (v_of s2)))
          (ensures eq (do (do (concrete_merge (v_of lca) (v_of s3') (v_of s2')) last1) last2) 
                       (concrete_merge (v_of lca) (v_of s3) (v_of s2))) = ()*)
