...
let linearizable_s1_01 (lca s1 s2:st)
  : Lemma (requires is_prefix (ops_of lca) (ops_of s1) /\
                    ...
                    (forall id id1. mem_id id (ops_of lca) /\ mem_id id1 (diff (ops_of s1) (ops_of lca)) ==> lt id id1) /\
                    (forall id id1. mem_id id (ops_of lca) /\ mem_id id1 (diff (ops_of s2) (ops_of lca)) ==> lt id id1)) 
          (ensures (exists l. interleaving_predicate l lca s1 s2)) = admit();
...
let interleaving_helper_inv1_len2 (lca s1 s2 l':log)
  : Lemma
...
let interleaving_helper_inv1_len1 (lca s1 s2 l':log)
  : Lemma
      (requires Seq.length s1 > 0 /\ Seq.length s2 > 0 /\
                is_prefix lca (fst (Seq.un_snoc s1)) /\ 
                is_prefix lca (fst (Seq.un_snoc s2)) /\
                is_interleaving l' (diff (fst (Seq.un_snoc s1)) lca) (diff (fst (Seq.un_snoc s2)) lca))
      (ensures (let _, last1 = un_snoc s1 in
                is_interleaving (Seq.snoc l' last1) (diff s1 lca) (diff s2 lca)))
  = admit();assert (is_interleaving l' (fst (Seq.un_snoc (diff s1 lca))) (fst (Seq.un_snoc (diff s2 lca))));
    let prefix1, last1 = Seq.un_snoc (diff s1 lca) in
...
let interleaving_helper_inv2_len2 (lca s1 s2 l':log)
  : Lemma
...
let interleaving_helper_inv2_len1 (lca s1 s2 l':log)
  : Lemma
...
let interleaving_s1_inv_len2 (lca s1 s2:st) (l':log)
  : Lemma (requires is_prefix (ops_of lca) (ops_of s1) /\
                    is_prefix (ops_of lca) (ops_of s2) /\
                    Seq.length (ops_of s1) > Seq.length (ops_of lca) /\ 
                    Seq.length (ops_of s2) > Seq.length (ops_of lca) /\ 
  ...
  let _, last1 = un_snoc (ops_of s1) in
  interleaving_helper_inv1_len2 (ops_of lca) (ops_of s1) (ops_of s2) l';
  linearizable_gt0 lca s1 s2;
  inverse_helper (v_of lca) l' last1

let interleaving_s1_inv_len1 (lca s1 s2:st) (l':log)
  : Lemma (requires is_prefix (ops_of lca) (ops_of s1) /\
                    is_prefix (ops_of lca) (ops_of s2) /\
                    Seq.length (ops_of s1) > Seq.length (ops_of lca) /\ 
                    Seq.length (ops_of s2) > Seq.length (ops_of lca) /\ 
          ...
          (ensures (let _, last1 = un_snoc (ops_of s1) in
                    ...
                    (exists l. interleaving_predicate l lca s1 s2))) = admit();
  let _, last1 = un_snoc (ops_of s1) in
  interleaving_helper_inv1_len1 (ops_of lca) (ops_of s1) (ops_of s2) l'; 
  linearizable_gt0 lca s1 s2;
  inverse_helper (v_of lca) l' last1

let interleaving_s2_inv_len2 (lca s1 s2:st) (l':log)
  : Lemma (requires is_prefix (ops_of lca) (ops_of s1) /\
                    is_prefix (ops_of lca) (ops_of s2) /\
                    Seq.length (ops_of s1) > Seq.length (ops_of lca) /\ 
                    Seq.length (ops_of s2) > Seq.length (ops_of lca) /\
          ...
          (ensures (let _, last2 = un_snoc (ops_of s2) in
                    ...
                    (exists l. interleaving_predicate l lca s1 s2))) = admit();
  let _, last2 = un_snoc (ops_of s2) in
  interleaving_helper_inv2_len2 (ops_of lca) (ops_of s1) (ops_of s2) l';
  linearizable_gt0 lca s1 s2;
  inverse_helper (v_of lca) l' last2

let interleaving_s2_inv_len1 (lca s1 s2:st) (l':log)
  : Lemma (requires is_prefix (ops_of lca) (ops_of s1) /\
                    is_prefix (ops_of lca) (ops_of s2) /\
                    Seq.length (ops_of s1) > Seq.length (ops_of lca) /\ 
                    Seq.length (ops_of s2) > Seq.length (ops_of lca) /\
          ...
          (ensures (let _, last2 = un_snoc (ops_of s2) in
                    ...
                    (exists l. interleaving_predicate l lca s1 s2))) = admit();
  let _, last2 = un_snoc (ops_of s2) in
  interleaving_helper_inv2_len1 (ops_of lca) (ops_of s1) (ops_of s2) l';
  linearizable_gt0 lca s1 s2;
  inverse_helper (v_of lca) l' last2

let linearizable_s1_gt0_pre_len2 (lca s1 s2:st)
  : Lemma (requires is_prefix (ops_of lca) (ops_of s1) /\
                    is_prefix (ops_of lca) (ops_of s2) /\
                    Seq.length (ops_of s1) > Seq.length (ops_of lca) /\
                    Seq.length (ops_of s2) > Seq.length (ops_of lca) /\
  ...
  admit();let inv1 = inverse_st s1 in 
...
let linearizable_s1_gt0_pre_len1 (lca s1 s2:st)
  : Lemma (requires is_prefix (ops_of lca) (ops_of s1) /\
                    is_prefix (ops_of lca) (ops_of s2) /\
                    Seq.length (ops_of s1) > Seq.length (ops_of lca) /\
                    Seq.length (ops_of s2) > Seq.length (ops_of lca) /\
          ...
          (ensures (let inv1 = inverse_st s1 in 
                    ...
                    concrete_merge_pre (v_of lca) (v_of inv1) (v_of inv2) /\
                    (forall id id1. mem_id id (ops_of lca) /\ mem_id id1 (diff (ops_of inv1) (ops_of lca)) ==> lt id id1) /\
                    (forall id id1. mem_id id (ops_of lca) /\ mem_id id1 (diff (ops_of inv2) (ops_of lca)) ==> lt id id1) /\
                    (forall id. mem_id id (diff (ops_of inv1) (ops_of lca)) ==> not (mem_id id (diff (ops_of inv2) (ops_of lca)))))) =admit();
...
let linearizable_s2_gt0_pre_len2 (lca s1 s2:st)
  : Lemma (requires is_prefix (ops_of lca) (ops_of s1) /\
                    is_prefix (ops_of lca) (ops_of s2) /\
                    Seq.length (ops_of s1) > Seq.length (ops_of lca) /\ 
                    Seq.length (ops_of s2) > Seq.length (ops_of lca) /\
  ...
  admit();let inv2 = inverse_st s2 in 
  lem_inverse (ops_of lca) (ops_of s2);
  merge_inv_prop lca s1 s2; 
  lemma_split (ops_of inv2) (Seq.length (ops_of lca));
  lem_diff (ops_of inv2) (ops_of lca); 
...
let linearizable_s2_gt0_pre_len1 (lca s1 s2:st)
  : Lemma (requires is_prefix (ops_of lca) (ops_of s1) /\
                    is_prefix (ops_of lca) (ops_of s2) /\
                    Seq.length (ops_of s1) > Seq.length (ops_of lca) /\ 
                    Seq.length (ops_of s2) > Seq.length (ops_of lca) /\
...
val convergence (lca s1 s2 s1':concrete_st)
