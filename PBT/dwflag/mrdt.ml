...
let mrdt_do s e =
  match e with
  | _, rid, Disable -> Ewflag.update rid 
                                    (function
                                    | None -> Some (1, true)
                                    | Some _ -> Some (fst (Ewflag.find_opt rid s |> Option.value ~default:(0, true)) + 1, true)) 
                                    s
  | _, _, Enable -> Ewflag.map (fun (c,_) -> (c, false)) s
...
let mrdt_merge (l:state) (a:state) (b:state) : state =
  ...
  let u = IntSet.fold (fun k m -> Ewflag.add k (0, true) m) keys Ewflag.empty in
  let m = IntSet.fold (fun k u_map ->
    let l' = Ewflag.find_opt k l |> Option.value ~default:(0, true) in
    let a' = Ewflag.find_opt k a |> Option.value ~default:(0, true) in
    let b' = Ewflag.find_opt k b |> Option.value ~default:(0, true) in
