...
let test_config =
  let c = apply init_config 1 (gen_ts (), 1, Write 'a') in
  let c1 = merge c 0 1 in
  let c2 = apply c1 2 (gen_ts (), 2, Write 'b') in
  let c3 = apply c2 1 (gen_ts (), 1, Write 'b') in
  let c4 = apply c3 2 (gen_ts (), 2, Write 'a') in
