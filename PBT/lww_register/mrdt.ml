...
type state = (int * char) (* timestamp, value *)
type op = 
  | Write of char
...
let mrdt_merge _ a b =
  if fst a > fst b then a else b

let rc e1 e2 =
  if get_ts e1 > get_ts e2 then Snd_then_fst else Fst_then_snd
...
let print_st (s:state) =
  Printf.printf "(%d,%c)" (fst s) (snd s)
  
let string_of_event ((t, r, o):event) =
  let Write v = o in
  Printf.sprintf "(%d, %d, (%s %c))" t r "Write" v
