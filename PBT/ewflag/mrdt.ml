...
let get_en_cnt (s:state) =
    Ewflag.fold (fun _ (c, _) acc -> acc + c) s 0
  
let read (r:repId) (c:config) =
  (get_en_cnt (VerMap.find (RepIdMap.find r c.h) c.n), false)

let sim (r:repId) (c:config) =
  let l = List.length (List.filter (fun e -> let (_,_,o) = e in is_enable o) 
            (VerMap.find (RepIdMap.find r c.h) c.l)) in
  debug_print "\nNo. of E events at R%d is %d" r l;
  fst (read r c) = l
...
let sim_check (r:repId) (c:config) =
  debug_print "\nSim check on R%d" r;
  let s = 
    match Ewflag.find_opt r (VerMap.find (RepIdMap.find r c.h) c.n) with
    | Some s -> s
    | None -> (0,false) in
  debug_print "\nState at R%d is (%d,%b)" r (fst s) (snd s);
  sim r c
...
let createBranch (c:config) (srcRid:repId) (dstRid:repId) : config =
  ...
  let s_r0 = 
    match Ewflag.find_opt dstRid (VerMap.find (RepIdMap.find 0 new_c.h) new_c.n) with
    | Some s -> s
    | None -> (0, false) in
  let s_r1 = 
    match Ewflag.find_opt dstRid (VerMap.find (RepIdMap.find 1 new_c.h) new_c.n) with
    | Some s -> s
    | None -> (0, false) in
  debug_print "\n!!check st at R0 is (%d,%b)" (fst s_r0) (snd s_r0); 
  debug_print "\n!!check st at R1 is (%d,%b)" (fst s_r1) (snd s_r1); 
  print_dag new_c;
