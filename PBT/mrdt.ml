...
type dag = { 
  ...
  edgeType : edge list;  (* List of edges *)
}
...
let add_edge (g:dag) (src:verNo) (label:edgeType) (dst:verNo) : dag = {
  ...
  edgeType = {src; label; dst} :: g.edgeType;
}

(* Check if an edge exists between two vertices *)
let edge_exists (g:dag) (src:verNo) (dst:verNo) : bool =
  List.exists (fun e -> e.src = src && e.dst = dst) g.edgeType
...
let initVis = VisSet.empty

(* Initial configuration *)
let init_config = { n = initN; h = initH; l = initL; g = initG; vis = initVis }

(* CreateBranch function *)
let createBranch (c:config) (srcRid:repId) (dstRid:repId) (newVer:verNo) : config =
  if srcRid = dstRid then failwith "Destination replica must be different from source replica";
  if version_exists c.g.vertices newVer then failwith "New version already exists in the configuration";
  ...
  { n = newN; h = newH; l = newL; g = newG; vis = c.vis }
...
let apply (c:config) (srcRid:repId) (newVer:verNo) (o:op) : config = 
  ...
  { n = newN; h = newH; l = newL; g = newG; vis = newVis}

  (* Function to find the ancestors of a given version *)
let rec find_ancestors (c:config) (v:verNo) : VerSet.t =
  ...
  List.fold_left (fun acc edge -> 
    ...
    else acc) (VerSet.add v VerSet.empty) c.g.edgeType

(* Check if one version is an ancestor of another *)
(*let is_ancestor (c:config) (ancestor:verNo) (v:verNo) =
  ancestor = v || VerSet.mem ancestor (find_ancestors c v)*)

(* Function to find the LCA of two versions *)
let find_lca (c:config) (v1:verNo) (v2:verNo) : verNo option =
  (*if is_ancestor c v1 v2 then Some v1
  else if is_ancestor c v2 v1 then Some v2
  else*)
    ...
    if VerSet.is_empty ca then None
    else Some (VerSet.max_elt ca)
...
let merge (c:config) (r1:repId) (r2:repId) (newVer:verNo) : config =
  ...
  let m = match lca with
    ...
    | Some vl -> Printf.printf "LCA is %d\n" vl; let m = merge vl v1 v2 in
  Printf.printf "merge of %d %d %d is %d\n" vl v1 v2 m; m in
...
let str_of_edge = function
  | CreateBranch (r1, r2) -> Printf.sprintf "CreateBranch r%d from r%d" r2 r1
  | Apply (r, _) -> Printf.sprintf "Apply r%d, op: %s" r "Incr"
  | Merge (r1, r2) -> Printf.sprintf "Merge r%d into r%d" r2 r1

let print_vertex v = Printf.printf "Vertex: %d\n" v
let print_edge e = Printf.printf "%d --> [%s] --> %d\n" e.src (str_of_edge e.label) e.dst

let print_vertex_with_state (c:config) (v:verNo) =
  let state = c.n v in
  Printf.printf "v%d => %d\n" v state

let print_dag_with_states (c:config) =
  Printf.printf "Vertices with their States:\n";
  VerSet.iter (print_vertex_with_state c) c.g.vertices;
  Printf.printf "\nEdges:\n";
  List.iter print_edge (List.rev c.g.edgeType)
