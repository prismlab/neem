...
let concrete_st = M.t string concrete_st_v
...
let sel (s:concrete_st) (k:string) =
  if M.contains s k then M.sel s k else init_st_v
...
let eq (s1 s2:concrete_st) =
 (forall k. (M.contains s1 k = M.contains s2 k) /\ eq_v (sel s1 k) (sel s2 k))
...
let do (s:concrete_st) (o:op_t) : concrete_st =
  let k = get_key o in
  M.upd s k (do_v (sel s k) (get_op_v o))
