module Framework_do_pre_first

open FStar.Seq
open App_do_pre_first
open SeqUtils
...
let interleaving_predicate (l:log) (lca s1:st)
  (s2:st{merge_pre (v_of lca) (v_of s1) (v_of s2) /\ 
...
let rec linearizable_s1_0''_base (lca s1 s2':st) (last2:op_t)
  ...
  if length (ops_of lca) = 0 then
    ...
    (let l' = inverse_st lca in
     ...
     linearizable_s1_0''_base_do_pre lca s1 s2' last2;
     linearizable_s1_0''_base_merge_pre lca s1 s2' last2; 
...
let rec linearizable_s1_0'' (lca s1 s2':st) (last2:op_t)
  ...
  if ops_of s2' = ops_of lca then
    ...
    linearizable_s1_0''_do_pre lca s1 s2' last2;
    let s2'' = inverse_st s2' in
    distinct_snoc_inv (ops_of s2') last2;
    ...
    linearizable_s1_0''_merge_pre lca s1 s2' last2;
...
let rec linearizable_gt0_s2'_s10 (lca s1 s2:st) (last1 last2:op_t)
  ...
  if ops_of s2 = ops_of lca then
    ...
    (assert (length (ops_of s2) > length (ops_of lca)); 
     linearizable_gt0_s2'_do_pre lca s1 s2 last1 last2;
     linearizable_gt0_s2'_merge_pre lca s1 s2 last1 last2;
     let s2' = inverse_st s2 in
     lem_inverse (ops_of lca) (ops_of s2);
     lastop_diff (ops_of lca) (ops_of s2);
...
let rec linearizable_gt0_s1'_s20 (lca s1 s2:st) (last1 last2:op_t)
  ...
  if ops_of s1 = ops_of lca then
    ...
    (assert (length (ops_of s1) > length (ops_of lca));
     linearizable_gt0_s1'_do_pre lca s1 s2 last1 last2;
     linearizable_gt0_s1'_merge_pre lca s1 s2 last1 last2;
     let s1' = inverse_st s1 in
     lem_inverse (ops_of lca) (ops_of s1);
     lastop_diff (ops_of lca) (ops_of s1);
...
let linearizable_gt0 (lca s1 s2:st)
  ...
  if First? (resolve_conflict last1 last2) then
    linearizable_gt0_s1s2 lca s1 s2
  else if First_then_second? (resolve_conflict last1 last2) then
