...
let do (s:concrete_st) (op:op_t{do_pre s op}) 
  : (r:concrete_st{(*//(Add_after? (snd op) ==> (forall id. mem_id_s id (fst r) <==> mem_id_s id (fst s) \/ id = fst op)) /\
   ...
   match op with
  |(ts, Add_after after_id ele) -> //if after_id <> 0 && not (mem_id_s after_id (fst s)) then s else
                                   (S.add (ts, (after_id, ele)) (fst s), snd s)
...
let rec invalid_exec1 (lca s1 s2:st) (last1 last2:op_t)
  ...
  //assume ((fst last1 = get_rem_id last2) ==> ~ (Add_after? (snd last1) /\ Remove? (snd last2)));
  if ops_of lca = ops_of s1 then
    (if ops_of lca = ops_of s2 then admit()
     else 
          ...
          (assume (not (mem_id_s (get_rem_id last2) (fst (v_of s2'))));
           if Add_after? (snd lastop2) && fst lastop2 = get_rem_id last2 then 
             (lem_diff (snoc (ops_of s2) last2) (ops_of lca);
              ...
              assume (mem_id (fst lastop2) (diff (ops_of s2) (ops_of lca)));
              assume (not (mem_id (fst lastop2) (ops_of lca)));
              assume (mem_id (fst lastop2) (ops_of s2));
              lemma_append_count_assoc_fst (ops_of s2) (create 1 last2);
              assume (mem_id (fst lastop2) (snoc (ops_of s2) last2)); 
              lemma_append_count_assoc_fst (snoc (ops_of s2) last2) (create 1 last2);
              assume (mem_id (fst lastop2) (diff (snoc (ops_of s2) last2) (ops_of lca)));
              lem_diff (snoc (ops_of s1) last1) (ops_of lca);
              assume (mem_id (fst last1) (diff (snoc (ops_of s1) last1) (ops_of lca))); //done
              ()) 
           else 
             (admit();assume (do_pre (v_of s2') last2); 
              assume (consistent_branches lca (do_st s1 last1) (do_st s2' last2)); //can be done
    ...
    (admit()(*;let s1' = inverse_st s1 in 
     ...
     if Add_after? (snd lastop1) && fst lastop1 <> get_after_id last1 then 
       (admit();assume (not (mem_id_s (fst last1) (fst (v_of s1'))));
        //assume (~ (get_after_id last1 = 0) ==> mem_id_s (get_after_id last1) (fst (v_of s1')));
        assume (do_pre (v_of s1') last1);
        assume (consistent_branches lca (do_st s1' last1) (do_st s2 last2)); //can be done
        invalid_exec1 lca s1' s2 last1 last2)
     else if Add_after? (snd lastop1) && fst lastop1 = get_after_id last1 then  //check this case with full do_pre for add
       (admit();assume (not (mem_id_s (fst last1) (fst (v_of s1')))); 
        //assert (~ (get_after_id last1 = 0) ==> mem_id_s (get_after_id last1) (fst (v_of s1')));
        assume (do_pre (v_of s1') last1);
        assume (consistent_branches lca (do_st s1' last1) (do_st s2 last2)); //can be done
        invalid_exec1 lca s1' s2 last1 last2)
     else if Remove? (snd lastop1) && get_rem_id lastop1 <> fst last1 then
       (admit();assume (do_pre (v_of s1') last1); 
        assume (consistent_branches lca (do_st s1' last1) (do_st s2 last2)); //can be done
        invalid_exec1 lca s1' s2 last1 last2)
     else admit()*)) //Remove? (snd lastop1) && get_rem_id lastop1 = fst last1
