...
let rec get_ts'' (s:list (pos & string)) (t:S.t pos) 
  : Tot (t1:S.t pos{forall id. S.mem id t1 <==> S.mem id t \/ mem_id_s id s}) (decreases s) =
  match s with
  |[] -> t
  |x::xs -> get_ts'' xs (S.insert (fst x) t)

let get_ts (s:list (pos & string)) : Tot (S.t pos) =
  get_ts'' s S.empty
...
let do' (s:list (pos & string)) (op:op_t) 
  : (r:list (pos & string){(forall e. L.mem e r <==> (L.mem e s \/ e = op)) /\
                           (forall id. mem_id_s id r <==> mem_id_s id s \/ id = fst op)}) =
  (fst op, snd op)::s 
  
let subset_ts1 (s:list (pos & string)) (o:op_t) 
  : Lemma (ensures (S.subset (get_ts' (do' s o)) (S.insert (fst o) (get_ts' s)))) = ()*)
  
let lem_do (a b:concrete_st) (op:op_t)
   : Lemma (requires eq a b /\ do_pre a op)
           (ensures eq (do a op) (do b op)) = ()

let subset_ts (s:concrete_st) (o:op_t) 
...
(*let rec remove (s1:concrete_st) (ele:(pos * string)) 
  : Tot (r:concrete_st{forall e. L.mem e r <==> L.mem e s1 /\ e <> ele}) =
  match s1 with
  |[] -> []
...
let merge_pre l a b = 
  ...
  //(forall e. (L.mem e a /\ L.mem e b) ==> L.mem e l)
...
let concrete_merge (lca:concrete_st) (s1:concrete_st) (s2:concrete_st{merge_pre lca s1 s2}) 
  ...
  //assert (forall id. mem_id_s id lca ==> not (mem_id_s id la) /\ not (mem_id_s id lb));
