...
let one_op_inter_lca (l s1 s2 s3:concrete_st) (o1 ol oi:op_t)
  ...
  let e = get_ele o1 in
  if get_ele ol = get_ele oi && get_ele o1 = get_ele ol && Add? (snd (snd o1)) then () //done
  else if get_ele ol = get_ele oi && get_ele o1 <> get_ele ol && Add? (snd (snd o1)) then () //done
  else if get_ele ol <> get_ele oi && get_ele o1 = get_ele ol && Add? (snd (snd o1)) then () //done
  else if get_ele ol <> get_ele oi && get_ele o1 <> get_ele ol && Add? (snd (snd o1)) then () //done
  else if get_ele ol = get_ele oi && get_ele o1 = get_ele ol && Rem? (snd (snd o1)) then () //done
  else if get_ele ol = get_ele oi && get_ele o1 <> get_ele ol && Rem? (snd (snd o1)) then () //done
  else if get_ele ol <> get_ele oi && get_ele o1 = get_ele ol && Rem? (snd (snd o1)) then () //done
  else ()
...
let comm_inter_left (l s1 s2 s3:concrete_st) (o1 o2 ob ol o:op_t) 
  : Lemma (requires Either? (rc o1 o2) /\ distinct_ops o1 o2 /\ 
                    ...
                    eq (merge (do l ol) (do (do (do s1 ob) ol) o1) (do (do s2 ol) o2)) (do (do (do (do s3 ob) ol) o1) o2))
          (ensures eq (merge (do l ol) (do (do (do (do s1 o) ob) ol) o1) (do (do s2 ol) o2)) (do (do (do (do (do s3 o) ob) ol) o1) o2)) =
  assert (commutes_with o1 o2); 
  assert (eq (do (do (do (do (do s3 o) ob) ol) o1) o2) (do (do (do (do (do s3 o) ob) ol) o2) o1)); 
  assert (eq (do (do (do (do s3 ob) ol) o1) o2) (do (do (do (do s3 ob) ol) o2) o1));
  merge_comm (do l ol) (do (do s2 ol) o2) (do (do (do s1 ob) ol) o1);
  comm_inter_right l s2 s1 s3 o2 o1 ob ol o; 
  assert (eq (merge (do l ol) (do (do s2 ol) o2) (do (do (do (do s1 o) ob) ol) o1)) (do (do (do (do (do s3 o) ob) ol) o2) o1));
  merge_comm (do l ol) (do (do s2 ol) o2) (do (do (do (do s1 o) ob) ol) o1)

let comm_inter_lca_s (l s1 s2 s3:concrete_st_s) (o1 o2 ol:(nat & (nat & op_s)))
  : Lemma (requires ((Disable? (snd (snd o1)) /\ Disable? (snd (snd o2))) \/ (Enable? (snd (snd o1)) /\ Enable? (snd (snd o2)))) /\
                    Enable? (snd (snd ol)) /\
                    eq_s (merge_s l (do_s s1 o1) (do_s s2 o2)) (do_s (do_s s3 o1) o2))
          (ensures eq_s (merge_s (do_s l ol) (do_s (do_s s1 ol) o1) (do_s (do_s s2 ol) o2)) (do_s (do_s (do_s s3 ol) o1) o2)) =()

let comm_inter_lca1 (l s1 s2 s3:concrete_st) (o1 o2 ol oi:op_t)
  ...
  let e = get_ele o1 in
  if get_ele ol = get_ele oi && get_ele o1 = get_ele ol && Add? (snd (snd o1)) && Add? (snd (snd o2)) && get_ele o1 <> get_ele o2 then 
    ( 
    //if get_rid o1 = get_rid o2 && get_rid ol = get_rid oi && get_rid ol = get_rid o1 then () //done
    //if get_rid o1 = get_rid o2 && get_rid ol = get_rid oi && get_rid ol <> get_rid o1 then () //done
    //if get_rid o1 = get_rid o2 && get_rid ol <> get_rid oi && get_rid ol = get_rid o1 then () //done
    if get_rid o1 = get_rid o2 && get_rid ol <> get_rid oi && get_rid ol <> get_rid o1 then () //done
    //if get_rid o1 <> get_rid o2 && get_rid ol = get_rid oi && get_rid ol = get_rid o1 then () //done
  ...
  (*let e = get_ele ol in
  if e = get_ele o1 && get_ele o1 = get_ele o2 then 
    (assert (eq_s (sel_k (do (do s3 o1) o2) e) 
                 (do_s (do_s (sel_k s3 e) (get_op_s o1)) (get_op_s o2))); admit();
    if ((Disable? (snd (snd (get_op_s o1))) && Disable? (snd (snd (get_op_s o2)))) ||
       (Enable? (snd (snd (get_op_s o1))) && Enable? (snd (snd (get_op_s o2))))) then
       comm_inter_lca_s (sel_k l e) (sel_k s1 e) (sel_k s2 e) (sel_k s3 e) (get_op_s o1) (get_op_s o2) (get_op_s ol)
    else admit())*)
  //if e = get_ele o1 && get_ele o1 <> get_ele o2 && Add? (snd (snd o1)) && Add? (snd (snd o2)) then () //done
  //if e <> get_ele o1 && get_ele o1 = get_ele o2 && Add? (snd (snd o1)) && Add? (snd (snd o2)) then () //done
  //if e <> get_ele o1 && get_ele o1 <> get_ele o2 && Add? (snd (snd o1)) && Add? (snd (snd o2)) then () //done
  //if e = get_ele o1 && get_ele o1 <> get_ele o2 && Add? (snd (snd o1)) && Rem? (snd (snd o2)) then () //done
  //if e <> get_ele o1 && get_ele o1 = get_ele o2 && Add? (snd (snd o1)) && Rem? (snd (snd o2)) then () //done
  //if e <> get_ele o1 && get_ele o1 <> get_ele o2 && Add? (snd (snd o1)) && Rem? (snd (snd o2)) then () //done
  //if e = get_ele o1 && get_ele o1 <> get_ele o2 && Rem? (snd (snd o1)) && Rem? (snd (snd o2)) then admit() //TODO
  //if e <> get_ele o1 && get_ele o1 = get_ele o2 && Rem? (snd (snd o1)) && Rem? (snd (snd o2)) then () //done
  //if e <> get_ele o1 && get_ele o1 <> get_ele o2 && Rem? (snd (snd o1)) && Rem? (snd (snd o2)) then admit() //TODO
  //if e = get_ele o1 && get_ele o1 <> get_ele o2 && Rem? (snd (snd o1)) && Add? (snd (snd o2)) then admit() //TODO
  //if e <> get_ele o1 && get_ele o1 = get_ele o2 && Rem? (snd (snd o1)) && Add? (snd (snd o2)) the  () //done
  //if e <> get_ele o1 && get_ele o1 <> get_ele o2 && Rem? (snd (snd o1)) && Add? (snd (snd o2)) then () //done
   //else admit()

(*(requires Either? (rc o1 o2) /\ //distinct_ops o1 o2 /\ distinct_ops ol o1 /\ distinct_ops ol o2 /\
                    Add? (snd (snd ol)) /\
                    //(exists o'. Fst_then_snd? (rc o' ol)) /\
